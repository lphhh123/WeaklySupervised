...
def random_walk(x, y, w):
    ...
    eye_x = eye_x.unsqueeze(0).repeat(x.size(0), 1, 1)
    latent_z = F.softmax(torch.einsum('nkd,ntd->nkt', [y_norm, x_norm]) * 2.0, 1)
    norm_latent_z = latent_z / (latent_z.sum(dim=-1, keepdim=True) + 1e-9)
    
    # Calculate affinity matrix [B, T, T]
    affinity_mat = torch.einsum('nkt,nks->nts', [norm_latent_z, norm_latent_z])
    
    # Solve linear system [B, T, T] * [B, T, D] = [B, T, D]
    mat_inv_x = torch.linalg.solve(eye_x - (w ** 2) * affinity_mat, eye_x)
    y2x_sum_x = w * torch.einsum('nkt,nkd->ntd', [latent_z, y]) + x
    refined_x = (1 - w) * torch.einsum('ntk,ntd->ntd', [mat_inv_x, y2x_sum_x])    
