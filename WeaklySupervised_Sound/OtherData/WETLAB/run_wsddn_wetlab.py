...
# train one fold
# ============================================================
def train_wsddn_one_fold_wetlab(config, fold: int, exp_name: str = "wsddn_opportunity"):
...
def test_wsddn_one_fold_wetlab(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    """
    test_mode:
      - "test_window": test subject also slides by clip_sec
      - "test_full"  : run the full sequence once
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "WSDDN-wetlab-v1.0", "results": results, "external_data": {}},
...
def run_loso_wsddn_wetlab(config):
    ...
    for i, fold in enumerate(folds):
        ...
        # 1) train
        wsddn_ckpt = train_wsddn_one_fold_wetlab(config, fold, exp_name=config.get("exp_name", "wsddn_opportunity"))
