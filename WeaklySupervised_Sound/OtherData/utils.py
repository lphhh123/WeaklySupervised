...
def _majority_label_in_window(win_s: int, win_e: int, annos, win_len: int, min_frac: float = 0.0):
    """
    Count overlap frames per label within [win_s, win_e) and pick the majority label.
    - annos: [(s,e,lid), ...] sorted by s, intervals are [s,e)
    - min_frac: if max fraction < min_frac, return None (treat as background)
      default 0.0: any overlap assigns a label
    """
    if win_e <= win_s:
        return None
...
def load_mean_std_from_meanvar_json(
    ...
    """
    Read the provided JSON format (mean + var) and strictly validate meta fields
    to avoid mixing folds or mismatched parameters.
    """
...
def _uniform_pick_indices(n: int, k: int):
    """Uniformly pick k indices from a sequence of length n (covering the range)."""
    if k <= 0 or n <= 0:
        return []
    if k >= n:
        return list(range(n))
...
def build_gt_for_anet(loso_json_path, out_gt_path):
    """
    Change subset=Validation to test in loso json for ANETdetection.
    """
...
def featbox_to_time_seconds(
    ...
    """
    Proposal index space ([start_idx,end_idx) in T_global) -> seconds.
    """
...
def generate_proposal_boxes(
    ...
    Multi-scale fixed windows: scales from a discrete set in [min_sec, max_sec]
    controlled by sec_resolution.
    """
...
class GlobalBackboneWrapper(nn.Module):
    ...
    def forward(self, x_raw: torch.Tensor, return_info: bool = False):
        """
        x_raw: [B, C, T_frames] (during training, T_frames is usually 900)
        return:
