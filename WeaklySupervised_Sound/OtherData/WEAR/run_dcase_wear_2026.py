# OtherData/WEAR/run_dcase_wear.py
# -*- coding: utf-8 -*-
import torch
...
os.environ["CUDA_VISIBLE_DEVICES"] = "7"  # 根据实际情况调整显卡ID
...
# ============================================================
# 2) Train one fold (WEAR)
# ============================================================
def train_dcase_one_fold_wear(config, fold: int, exp_name: str = "dcase_wear"):
    ...
    print(f"\n[Train WEAR] fold={fold} | clip={clip_sec}s | samples={len(train_ds)}")
...
# ============================================================
# 3) Test one fold (WEAR)
# ============================================================
@torch.no_grad()
def test_dcase_wear(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    print(f"\n[Test] Searching thresholds for Fold {fold} ({test_mode})...")
...
# 4) Main Runner
# ============================================================
def run_loso_dcase_wear(config):
    ...
    clip_sec = config.get("clip_sec", 60.0)
    suffix = f"_{int(clip_sec)}s"
    ...
    for fold in folds:
        # 1. Train
        ckpt = train_dcase_one_fold_wear(config, fold, exp_name=config["exp_name"])

        # 2. Test Window
        mAPs_win, avg_mAP_win, th_win = test_dcase_wear(config, ckpt, fold, test_mode="test_window")

        # 3. Test Full
        mAPs_full, avg_mAP_full, th_full = test_dcase_wear(config, ckpt, fold, test_mode="test_full")
        ...
        fold_result = {
            ...
            "window_mode": {
                ...
                "mAPs": mAPs_win.tolist() if mAPs_win is not None else []
            },
            "full_mode": {
                "best_threshold": float(th_full),
                "avg_mAP": float(avg_mAP_full),
                "mAPs": mAPs_full.tolist() if mAPs_full is not None else []
            }
        }
...
if __name__ == "__main__":
    ...
    config = {
        "seed": 2026,
        "exp_name": "dcase_crnn_wear",
        "model_type": "CRNN",

        "dataset_dir": "/home/lipei/TAL_data/wear/",
        "checkpoint_dir": "/home/yinjiaxi/wstal/WeaklySupervised-master/checkpoints/wear_dcase_10_60s_2026",
        "result_root": "/home/yinjiaxi/wstal/WeaklySupervised-master/result/wear_dcase_10_60s_2026/",
