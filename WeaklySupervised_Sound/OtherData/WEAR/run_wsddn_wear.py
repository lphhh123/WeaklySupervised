...
# train one fold
# ============================================================
def train_wsddn_one_fold_wear(config, fold: int, exp_name: str = "wsddn_opportunity"):
...
# ============================================================
@torch.no_grad()
def test_wsddn_one_fold_wear(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    """
    test_mode:
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "WSDDN-wear-v1.0", "results": results, "external_data": {}},
...
def run_loso_wsddn_wear(config):
    ...
    for i, fold in enumerate(folds):
        ...
        # 1) train
        wsddn_ckpt = train_wsddn_one_fold_wear(config, fold, exp_name=config.get("exp_name", "wsddn_opportunity"))

        # 2) test_window
        mAPs_w, avg_w, pred_w = test_wsddn_one_fold_wear(config, wsddn_ckpt, fold=fold, test_mode="test_window")

        mAPs_f, avg_f, pred_f = test_wsddn_one_fold_wear(config, wsddn_ckpt, fold=fold, test_mode="test_full")
