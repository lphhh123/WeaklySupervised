...
def npy_windows_to_raw_frames(
    ...
    """
    npy: shape [N, num_sensors*win_samples], flatten order is sensor-major:
      sensor0(30) + sensor1(30) + ... + sensor112(30)

    npy windows: win_samples (=30) samples, overlap=0.5 -> stride=15 samples

    Restore continuous 30Hz raw:
      The first window uses all 30 frames,
      subsequent windows append only the newly appeared frames (last 15 frames).
...
def _compute_mean_var_across_subjects(
    ...
    """
    Compute per-channel mean/var from restored continuous raw sequence (30Hz).
    """
...
def ensure_all_loso_stats_json(
    ...
    """
    Ensure stats for all 4 folds exist via leave-one-out:
      raw/loso_norm_stats_json/loso_sbj_k_stats.json

    If stats dir or a fold json is missing, compute and write it.
    """
    for k in folds:
...
def load_fold_mean_std_from_json(
    ...
    """
    Read fold stats json, return mean/std, and validate meta to avoid misuse.
    """
