...
class AFBlock1D_7s(nn.Module):
    """
    ActionFormer-style 1D residual conv block:
...
class ActionFormerBackbone_7s(nn.Module):
    ...
    Simplified ActionFormer-style backbone:
      Conv1d (local) + multi-layer dilated conv blocks + Transformer (global).
      Input: (B, 30, 478)
...
class PositionalEncoding1D(nn.Module):
    """Standard 1D Transformer positional encoding with dynamic generation when T > max_len."""
...
class TemporalTransformer1D(nn.Module):
    ...
    1D Transformer based on nn.TransformerEncoder:
      - Input (B, C, T) is internally transposed to (B, T, C) for self-attention.
    """
    def __init__(
        self,
        d_model: int,
