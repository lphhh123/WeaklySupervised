...
def calculate_average_metrics():
    ...
    valid_files_count = 0
    print(f"Processing files under: {base_dir}")

    for file_path in file_paths:
        if not os.path.exists(file_path):
            print(f"Warning: file not found {file_path}, skipped.")
            continue
    ...
    try:
        ...
        print(f"Success. Averages saved to: {output_path}")
        print(f"Included categories: {list(final_results.keys())}")
    except Exception as e:
