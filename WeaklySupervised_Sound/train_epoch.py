...
def train_wsddn_imu(config, exp_name="wsddn"):
    ...
    print(f"Training samples: {len(train_dataset)}, batch size: {config['training']['batch_size']}")

    train_backbone = bool(config["training"].get("train_backbone", False))
    if not backbone_loaded:
        train_backbone = True
        print("[Train] Pretrained checkpoint missing -> force train_backbone=True (random init and train)")
    ...
    print(f"Spatial reg weight λ = {lambda_reg}, IoU threshold = {iou_thresh}")
    ...
    for epoch in range(num_epochs):
        ...
        if avg_loss < best_loss:
            ...
            print(f"Saved best model to: {ckpt_path} (loss: {best_loss:.4f})")
...
def train_pcl_imu(config, exp_name="pcl_imu"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[PCL/OICR] Training device: {device}")
    print(f"[PCL/OICR] Experiment name: {exp_name}")
    ...
    print(f"[PCL/OICR] Training samples: {len(train_dataset)}")

    train_backbone = bool(config["training"].get("train_backbone", False))
    if not backbone_loaded:
        train_backbone = True
        print("[PCL/OICR-Train] Pretrained checkpoint missing -> force train_backbone=True (random init and train)")
    ...
    for epoch in range(num_epochs):
        ...
        current_lr = scheduler.get_last_lr()[0]
        print(f"[PCL/OICR] Epoch {epoch+1} | Avg loss: {epoch_loss:.4f}")
    ...
    print(f"[PCL/OICR] Training complete, best_loss={best_loss:.4f}")
    return ckpt_path
