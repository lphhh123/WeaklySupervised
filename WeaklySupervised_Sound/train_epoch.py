...
def train_wsddn_imu(config, exp_name="wsddn"):
    ...
    print(f"Message：{device}")
    print(f"Message：{exp_name}")
    ...
    print(f"Message：{len(train_dataset)}，Message：{config['training']['batch_size']}")


    train_backbone = bool(config["training"].get("train_backbone", False))
    if not backbone_loaded:
        train_backbone = True
        print("[Train] Message -> Message train_backbone=True（Message）")
    ...
    print(f"Message λ = {lambda_reg}, IoU Message = {iou_thresh}")
    ...
    for epoch in range(num_epochs):
        ...
        if avg_loss < best_loss:
            ...
            print(f"Message：{ckpt_path}（Message：{best_loss:.4f}）")


        with open(train_log_path, "a") as f:
            f.write(f"{epoch + 1},{avg_loss:.6f},{best_loss:.6f},{current_lr:.6f}\n")

        print(f"Epoch {epoch + 1} | Message：{avg_loss:.4f} | Message：{current_lr:.6f}")
...
def train_pcl_imu(config, exp_name="pcl_imu"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[PCL/OICR] Message：{device}")
    print(f"[PCL/OICR] Message：{exp_name}")
    ...
    print(f"[PCL/OICR] Message：{len(train_dataset)}")


    train_backbone = bool(config["training"].get("train_backbone", False))
    if not backbone_loaded:
        train_backbone = True
        print("[PCL/OICR-Train] Message -> Message train_backbone=True（Message）")
    ...
    for epoch in range(num_epochs):
        ...
        current_lr = scheduler.get_last_lr()[0]
        print(f"[PCL/OICR] Epoch {epoch+1} | Message: {epoch_loss:.4f}")
    ...
    print(f"[PCL/OICR] Message，best_loss={best_loss:.4f}")
    return ckpt_path
