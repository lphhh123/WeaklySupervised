...
def run_pure_test(config, checkpoint_path, test_mode="full"):
    ...
    if not os.path.exists(checkpoint_path):
        print(f"❌ Message: Message {checkpoint_path}")
        return
    ...
    results = {}
    print(f"\n🔍 Message: {test_mode.upper()} | Threshold: {current_threshold} | NMS IoU: {current_iou}")
    ...
    print(f"✅ [{test_mode}] Message: {save_path}")
...
def main():
    ...
    print(f"🔹 Message (Total): {total_params:,}")
    print(f"🔹 Message (Trainable): {trainable_params:,}")
    print(f"--------------------------------------------------")
    print(f"🔸 CNN Message: {cnn_params:,} ({cnn_params / total_params:.1%})")
    print(f"🔸 RNN/GRU Message: {rnn_params:,} ({rnn_params / total_params:.1%})")
    print(f"🔸 Attention Message: {att_params:,} ({att_params / total_params:.1%})")
    print(f"🔹 Message: {total_params * 4 / (1024 ** 2):.2f} MB")
    print("=" * 50 + "\n")
    # ===============================
