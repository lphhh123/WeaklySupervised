...
def run_pure_test(config, checkpoint_path, test_mode="full"):
    ...
    results = {}
    print(f"\nMode: {test_mode.upper()} | Threshold: {current_threshold} | NMS IoU: {current_iou}")
    ...
    print(f"[{test_mode}] Results saved to: {save_path}")
...
def main():
    ...
    print("\nCRNN model structure and parameter summary:")
    from models.DCASE_CRNN_XRFV2 import CRNN
    ...
    print(f"CNN layers: {cnn_params:,} ({cnn_params / total_params:.1%})")
    print(f"RNN/GRU layers: {rnn_params:,} ({rnn_params / total_params:.1%})")
    print(f"Attention layers: {att_params:,} ({att_params / total_params:.1%})")
