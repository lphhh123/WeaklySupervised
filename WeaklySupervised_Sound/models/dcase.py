...
class IMUSpecAugment(nn.Module):
    ...
    def forward(self, x):
        ...
        x_aug = x.clone()
        _, _, time_dim, sensor_dim = x_aug.shape
...
class CRNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if rnn_type == "BGRU":
            ...
            raise NotImplementedError("Only BGRU supported for CRNN for now")
    ...
    def _get_logits(self, x, pad_mask, classes_mask=None):
        ...
        return self._get_logits_one_head(x, pad_mask, self.dense, dense_softmax, classes_mask)
    ...
    def forward(self, x, pad_mask=None, embeddings=None, classes_mask=None):
        # x input shape assumption: [Batch, Channels, Time]
