...
class IMU_PCL_OICR(nn.Module):
    """
    PCL/OICR head for IMU (with 1D SPP):
      Input: global_feat [B, C, T] or [B, T, C], proposal_boxes [B, P, 2], labels [B, C]
      Train: returns losses + per-stage scores
      Test: returns mil_score / refine_scores
    """
    def __init__(self,
                 feat_dim,
                 num_classes,
...
def _get_top_ranking_indices(probs, num_clusters=3, rng_seed=1):
    """
    K-means selection of high-score clusters from original PCL:
      probs: [P], scores for proposals in one class
    Return: indices belonging to high-score clusters
    """
...
def _get_graph_centers_1d(boxes, cls_prob, im_labels,graph_iou_thresh=0.5,max_pc_num=3,num_kmeans_cluster=3,rng_seed=1):
    """
    PCL graph-center selection for pseudo GT, 1D version.
    boxes: [P,2]
    cls_prob: [P,C]
...
def _get_highest_score_proposals_1d(boxes, cls_prob, im_labels):
    """
    OICR top-score proposal as pseudo GT, 1D version.
    """
    num_imgs, num_classes = im_labels.shape
    assert num_imgs == 1
    im_labels = im_labels[0].copy()
...
def _assign_clusters_1d(all_segments, proposals, im_labels,
    ...
    """
    Equivalent to original get_proposal_clusters:
...
def pcl_1d(
    ...
    """
    PCL 1D version: graph clustering + proposal clusters.
    """
...
def oicr_1d(
    ...
    """
    OICR 1D version: top-score proposal as pseudo GT.
    """
    cls_prob = cls_prob.copy()
    if cls_prob.shape[1] != im_labels.shape[1]:
        cls_prob = cls_prob[:, 1:]
