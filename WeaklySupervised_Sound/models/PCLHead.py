...
class Head_PCL(nn.Module):
    """
    Head_PCLMessageadapter+pcl/oicr，MessageTSSE+Mamba
    """
    def __init__(self,
                 feat_dim,
                 num_classes,
    ...
    def pool_proposals_1d_spp(self, global_feat, proposal_boxes):
        """
        Message 1D SPP Message proposal pooling
        global_feat:    [B, C, T]
        proposal_boxes: [B, P, 2] (start, end)
        Message:           [B, P, C * sum(levels)]
        """
    ...
    def forward(self, global_feat, proposal_boxes, labels=None):
        """
        global_feat: [B, C, T] Message [B, T, C]
        proposal_boxes: [B, P, 2]
        labels: [B, num_classes] (0/1)，Message
        """
        if global_feat.dim() != 3:
            raise ValueError("global_feat Message [B, C, T] Message [B, T, C]")
        ...
        if global_feat.size(1) == self.feat_dim:
            ...
            raise ValueError(
                f"global_feat Message feat_dim={self.feat_dim}, got {global_feat.shape}"
            )
        ...
        if self.training:
            if labels is None:
                raise ValueError("Message labels")
...
def map_boxes_input_to_feat(boxes: torch.Tensor, T_in: int, T_feat: int) -> torch.Tensor:
    """
    Message (0..T_in) Message feature Message (0..T_feat)
    boxes: [B,P,2]，end Message exclusive（Message pooling Message）
...
class Change_Block_PCL_Model(nn.Module):
    ...
    Message：
      ...
      proposals(Message or featMessage) -> head -> losses/scores
    """
...
class ROIHead_TSSELite(nn.Module):
    """
    Message 1 Message TSSE（Message Lspp=7，Message 1）
    """
...
class ROIHead_MambaLite(nn.Module):
    """
    Message MambaBackbone Message ROI encoder（Message，Message Lspp Message）
...
class ROIHead_TSSEMamba(nn.Module):
    """
    Message PCL/OICR Message ROI head（Message fc1+fc2）：

    Message：
      x_flat: [N, feat_dim * Lspp]   (N=B*P)
        - feat_dim = backboneMessage（Message512）
        - Lspp     = SPP Message bin Message（Message levels=(1,2,4) -> Lspp=7）
    Message：
      [N, hidden_dim] （Message4096），Message mil_head / refine_head

    Message：
      reshape -> (Message 1x1 conv Message512) -> TSSE -> Mamba(Message) -> mean pool -> MLP(->4096)
    """
...
class ROIHead_Transformer(nn.Module):
    """
    SPP Message x_flat=[N, feat_dim*Lspp]
    -> reshape Message [N, feat_dim, Lspp] Message token Message
    -> Transformer Message token mixing
...
class ROIHead_LSTM(nn.Module):
    """
    Message Lspp Message bin Message Lspp Message，Message LSTM Message token mixing。
    """
    def __init__(self, feat_dim: int, Lspp: int, hidden_dim: int = 4096):
        super().__init__()
        self.feat_dim = int(feat_dim)
