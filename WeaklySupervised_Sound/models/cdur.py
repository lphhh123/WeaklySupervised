...
class Block1d(nn.Module):
    def __init__(self, cin, cout, kernel_size=3, padding=1):
        ...
        self.block = nn.Sequential(
            nn.BatchNorm1d(cin),
            nn.Conv1d(cin, cout, kernel_size=kernel_size, padding=padding, bias=False),
...
class cATP(nn.Module):
    ...
    def forward(self, x):
        # x: [bs, time, dim]
        att = self.fc2(torch.tanh(self.fc1(x)))
        att = torch.softmax(att, dim=1)
        embedding_level = (x * att).sum(dim=1, keepdim=True)
        time_level = att
        return embedding_level, time_level
...
class cATPSDS(nn.Module):
    def __init__(self, inputdim, outputdim, **kwargs):
        ...
        kernels = [5, 5, 3]
        paddings = [2, 2, 1]
        self.dimensions = kwargs.get("dimensions", [46, 22, 92, 42, 82, 17, 13, 160, 74, 85])
        self.outputdim = outputdim
        features = nn.ModuleList([nn.BatchNorm1d(inputdim, eps=1e-4, momentum=0.01)])
        for h0, h1, kernel, padding in zip(filters, filters[1:], kernels, paddings):
            features.append(
                nn.Sequential(
                    nn.Conv1d(h0, h1, kernel_size=kernel, padding=padding, bias=False),
                    nn.BatchNorm1d(h1, eps=1e-4, momentum=0.01),
                    nn.ReLU(True),
                    nn.MaxPool1d(4),
                )
            )
        ...
        self.attentions = nn.ModuleList([cATP(self.dimensions[f]) for f in range(outputdim)])
        self.classifiers = nn.ModuleList([nn.Linear(self.dimensions[f], 1) for f in range(outputdim)])
...
def parse_poolingfunction(poolingfunction_name="mean", **kwargs):
    ...
    if poolingfunction_name == "linear":
        return LinearSoftPool(pooldim=1)
    if poolingfunction_name == "expalpha":
        return AutoExpPool(outputdim=kwargs["outputdim"], pooldim=1)
    if poolingfunction_name == "soft":
        return SoftPool(pooldim=1)
    if poolingfunction_name == "auto":
        return AutoPool(outputdim=kwargs["outputdim"])
    if poolingfunction_name == "attention":
        return AttentionPool(inputdim=kwargs["inputdim"], outputdim=kwargs["outputdim"])
    raise ValueError(f"Unknown pooling function: {poolingfunction_name}")
...
class AutoExpPool(nn.Module):
    def __init__(self, outputdim, pooldim=1):
        super().__init__()
        self.pooldim = pooldim
        self.alpha = nn.Parameter(torch.ones(outputdim))

    def forward(self, logits, decision):
        alpha = torch.clamp(self.alpha, min=0.01, max=10)
        w = torch.softmax(alpha[None, None, :] * decision, dim=self.pooldim)
        return torch.sum(decision * w, dim=self.pooldim)
...
class AutoPool(nn.Module):
    ...
    def forward(self, logits, decision):
        alpha = torch.clamp(self.alpha, min=0.01, max=10)
        w = torch.softmax(alpha[None, None, :] * logits, dim=self.pooldim)
        return torch.sum(decision * w, dim=self.pooldim)


class AttentionPool(nn.Module):
    def __init__(self, inputdim, outputdim, pooldim=1):
        super().__init__()
        self.pooldim = pooldim
        self.attention = nn.Linear(inputdim, outputdim)

    def forward(self, logits, decision):
        w = torch.softmax(self.attention(logits), dim=self.pooldim)
        return torch.sum(decision * w, dim=self.pooldim)
