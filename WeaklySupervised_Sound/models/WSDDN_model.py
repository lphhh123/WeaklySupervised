...
class WSDDN(nn.Module):
    ...
    def forward(self, x, proposal_boxes):
        """
        x: [B, C, T_global]  — backbone Message
        proposal_boxes: [B, P, 2]  — Message proposal Message [start, end]（Message）
        """
        B, C, T_global = x.shape
        P = proposal_boxes.shape[1]
...
class TemporalSPP1D(nn.Module):
    """
    1D Message Spatial Pyramid Pooling：
    Message [N, C, T]，Message [N, C * sum(levels)]
    Message levels=(1,2,4)，Message：
      - 1 Message bin -> [N, C*1]
      - 2 Message bin -> [N, C*2]
      - 4 Message bin -> [N, C*4]
    Message [N, C*(1+2+4)]
    ...
    @property
    def out_mul(self):
        """Message C * out_mul （out_mul = sum(levels)）"""
...
class WSDDNTransformerIMU(nn.Module):
    """
    WSDDN + Transformer Message（proposal Message 1D SPP）：
    - Message proposal Message backbone Message [1, C, T_p]
    - Message TemporalSPP1D Message 1D SPP -> [1, C * sum(levels)]
    - Message proposal Message token：[B, P, spp_out_dim] -> Message d_model
    - Message + Message -> TransformerEncoder
    - Message，Message WSDDN Message class / det Message
    - Message proposal_ctx Message feat_fc7，Message
    """
    def __init__(
        self,
        num_classes: int = 30,
    ...
    def _pool_proposals(self, x, proposal_boxes):
        """
        Message backbone Message x Message，Message proposal_boxes Message 1D SPP。
        x: [B, C, T_global]
        proposal_boxes: [B, P, 2] （start, end），Message（Message）
        Message:
    ...
    def forward(self, x, proposal_boxes):
        """
        x: [B, C, T_global]  —— Message backbone Message
        proposal_boxes: [B, P, 2] —— (start, end)，Message（Message：Message）
        """
        B, C, T_global = x.shape
        _, P, _ = proposal_boxes.shape
...
def generate_proposal_boxes(
    ...
    """
    Message（Messagestart_idxMessageend_idx）
    Message：Message（T_global）
    """
    proposal_boxes = []
...
def generate_proposals(imu_data, num_proposals=50, base_len=476, step=256, min_len=340, max_len=884):
    """
    :param imu_data: Message30s IMUMessage [30, 2048]
    :param base_len: 7sMessage（50Hz×7s=350 → Message2048*(7/30)≈476）
    :param step: Message（256）
    :param min_len/max_len: Message
    :return: 50Message [50, 30, T_proposal]
    """
...
def merge_overlapping_proposals(proposals, det_scores, overlap_thresh=0.5):
    """
    :param proposals: Message [50, 30, 884]
    :param det_scores: Message [50, num_classes]
    :return: Message25Message [25, 30, 884]
    """
