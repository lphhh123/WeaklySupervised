...
class WSDDN(nn.Module):
    ...
    def forward(self, x, proposal_boxes):
        """
        x: [B, C, T_global]  — backbone output
        proposal_boxes: [B, P, 2]  — each proposal [start, end] (time indices)
        """
        B, C, T_global = x.shape
        P = proposal_boxes.shape[1]
...
class TemporalSPP1D(nn.Module):
    """
    1D Spatial Pyramid Pooling:
    Given [N, C, T], output [N, C * sum(levels)]
    For levels=(1,2,4):
      - 1 bin -> [N, C*1]
      - 2 bins -> [N, C*2]
      - 4 bins -> [N, C*4]
    Concatenate to get [N, C*(1+2+4)]
    """
    ...
    @property
    def out_mul(self):
        """Output dimension is C * out_mul (out_mul = sum(levels))."""
...
class WSDDNTransformerIMU(nn.Module):
    """
    WSDDN + Transformer version (proposal first uses 1D SPP):
    - Extract each proposal from backbone features [1, C, T_p]
    - Apply TemporalSPP1D -> [1, C * sum(levels)]
    - Treat proposals as tokens: [B, P, spp_out_dim] -> project to d_model
    - Sort by center time + positional encoding -> TransformerEncoder
    - Restore original order and apply WSDDN class/det branches
    - Return proposal_ctx as feat_fc7 for spatial regularization
    """
    def __init__(
        self,
        num_classes: int = 30,
    ...
    def _pool_proposals(self, x, proposal_boxes):
        """
        Slice backbone feature x by proposal_boxes and apply 1D SPP.
        x: [B, C, T_global]
        proposal_boxes: [B, P, 2] (start, end), index (frame)
        Return:
    ...
    def forward(self, x, proposal_boxes):
        """
        x: [B, C, T_global]  — pretrained backbone output
        proposal_boxes: [B, P, 2] — (start, end) time indices (frames)
        """
        B, C, T_global = x.shape
        _, P, _ = proposal_boxes.shape
...
def generate_proposal_boxes(
    ...
    """
    Generate proposal segments (start_idx/end_idx in feature space).
    Core: map raw data scale to feature scale (T_global).
    """
    proposal_boxes = []
...
def generate_proposals(imu_data, num_proposals=50, base_len=476, step=256, min_len=340, max_len=884):
    """
    :param imu_data: single 30s IMU sample [30, 2048]
    :param base_len: 7s length (50Hz×7s=350 -> interpolated to 2048*(7/30)≈476)
    :param step: sliding stride (256)
    :param min_len/max_len: random length range
    :return: 50 proposal segments [50, 30, T_proposal]
    """
...
def merge_overlapping_proposals(proposals, det_scores, overlap_thresh=0.5):
    """
    :param proposals: proposal segments [50, 30, 884]
    :param det_scores: proposal confidence [50, num_classes]
    :return: merged 25 proposals [25, 30, 884]
    """
