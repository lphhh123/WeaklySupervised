...
from models.WSDDN_model import load_label_mapping, generate_proposal_boxes


class WeaklySupervisedXRFV2DatasetTrain(Dataset):
    def __init__(self, dataset_dir, mapping_path, split="train", num_proposals=50,
                 merge_proposals=False, T_global=30, use_airpods=True):
        ...
        with h5py.File(self.data_path, 'r') as f:
            self.data_len = f[self.modality].shape[0]
    ...
    def _preprocess_imu(self, sample_30s_np):
        """
        Fix: ensure sample_30s_np is used correctly inside the method.
        """
        sample_30s = torch.tensor(sample_30s_np, dtype=torch.float32).permute(1, 2, 0).reshape(30, -1)
        ...
        return (sample_30s - mean) / std

    def _preprocess_airpods(self, sample_air_np):
        """
        Fix: unify parameter naming logic.
        """
        sample_air = torch.tensor(sample_air_np, dtype=torch.float32)
        sample_air = torch.cat((sample_air[:, 3:6], sample_air[:, 6:9]), dim=-1).T
    ...
    def _init_h5(self):
        if self.h5_file is None:
            self.h5_file = h5py.File(self.data_path, 'r', swmr=True, rdcc_nbytes=1024 ** 2 * 16)

    def __getitem__(self, idx):
        self._init_h5()
        imu_30s = self.h5_file['imu'][idx]
        air_30s = self.h5_file['airpods'][idx] if self.use_airpods else None

        imu_feat = self._preprocess_imu(imu_30s)
        if self.use_airpods:
            air_feat = self._preprocess_airpods(air_30s)
            sample_30s = torch.cat([imu_feat, air_feat], dim=0)
        else:
            sample_30s = imu_feat

        proposal_boxes = torch.zeros((self.num_proposals, 2))
...
class WWADLDatasetTestSingle():
    ...
    def load_global_stats(self):
        """
        Load global mean and variance from file.
        If the current modality is missing, compute and update the file.
        """
        stats_path = os.path.join(self.dataset_dir, "global_stats.json")
...
class WeaklySupervisedXRFV2DatasetTest:
    def __init__(self,
        ...
        assert modality == 'imu', "WeaklySupervisedXRFV2DatasetTest currently only supports imu as the main modality"
        self.modality = modality
        self._debug_print = False
        ...
        if not self.use_airpods or self.airpods_dataset is None:
            for file_name, imu_iter in self.imu_dataset.dataset():
                if self._debug_print:
                    print("Debug: WeakIMUDatasetTest IMU only")
            ...
            for (imu_name, imu_iter), (air_name, air_iter) in zip(imu_files, air_files):
                assert imu_name == air_name, f"IMU/AirPods file name mismatch: {imu_name} vs {air_name}"
                ...
                def merged_iter(imu_iter=imu_iter, air_iter=air_iter):
                    for (imu_clip, seg_imu), (air_clip, seg_air) in zip(imu_iter, air_iter):
                        ...
                        # if nonlocal_first:
                        #     print("Debug: WeakIMUDatasetTest IMU + AirPods")
...
class WWADL_imu(WWADLBase):
    ...
    Data dimensions:
    - Data shape: (2900, 5, 6)
        - First dimension (2900): number of samples (e.g., time steps)
        - Second dimension (5): number of devices (5 IMU devices, see name_to_id)
        - Third dimension (6): IMU feature dimensions (e.g., accel + gyro axes)

    name_to_id maps device positions to indices:
        - 'glasses': 0
        - 'left hand': 1
        - 'right hand': 2
        - 'left pocket': 3
    ...
    def __init__(self, file_path, receivers_to_keep=None, new_mapping=None):
        """
        Initialize the IMU data handler and optionally keep specific devices.
        Args:
            file_path (str): data file path
            devices_to_keep (list, optional): device names to keep (e.g., ['glasses', 'left hand']).
        """
    ...
    def load_data(self, file_path):
        """
        Load IMU data and complete preprocessing.
    ...
    def retain_devices(self, devices_to_keep):
        """
        Filter and keep specific device dimensions.

        Args:
            devices_to_keep (list): device names to keep
        """
        name_to_id = {
            'glasses': 0,
            'left hand': 1,
...
class FullBackboneWrapper1D(nn.Module):
    """
    Extend a backbone that only accepts fixed-length windows to full sequences:
    - Slide windows through the backbone to get per-window features f: [B,D,Lw]
    - Map each window back to the global feature timeline via stride (frames_per_bin)
    - Average overlaps for stability, yielding global_feat: [B,D,T_global]
    """
    ...
    def recalibrate(self, device, dtype):
        """
        Estimate effective stride by measuring output-length changes.
        This is stable for CNN/Conv stacks: frames_per_bin ≈ delta_raw / delta_L
        """
        T0 = self.win_len
...
def load_h5(filepath):
    def recursively_load_group_to_dict(h5file, path):
        """
        Recursively load HDF5 groups and datasets into a nested dict.
        """
        result = {}
        group = h5file[path]
...
def handle_nan_and_interpolate(data, window_len, target_len):
    """
    Interpolate while handling NaN values before interpolation.
    Args:
        data (np.ndarray): input data with shape (window_len, ...)
        window_len (int): original sequence length.
        target_len (int): target sequence length.
    Returns:
        np.ndarray: interpolated data with time dimension target_len.
    """
