...
class WeaklySupervisedXRFV2DatasetTrain(Dataset):
    ...
    def _preprocess_imu(self, sample_30s_np):
        """
        Message：Message sample_30s_np Message
        """
...
class WeaklySupervisedXRFV2DatasetTest:
    def __init__(self,
        ...
        assert modality == 'imu', "WeaklySupervisedXRFV2DatasetTest Message imu Message"
        self.modality = modality
        self._debug_print = False
    ...
    def dataset(self):
        ...
        if not self.use_airpods or self.airpods_dataset is None:
            ...
            for (imu_name, imu_iter), (air_name, air_iter) in zip(imu_files, air_files):
                assert imu_name == air_name, f"IMU/AirPods Message: {imu_name} vs {air_name}"
...
class WWADL_imu(WWADLBase):
    """
    Message:
    - Message shape: (2900, 5, 6)
        - Message (2900): Message（Message）
        - Message (5): Message（5MessageIMUMessage，Message name_to_id）
        - Message (6): IMUMessage（Message6Message）
    ...
    def __init__(self, file_path, receivers_to_keep=None, new_mapping=None):
        """
        Message IMU Message，Message
        Args:
            file_path (str): Message
            devices_to_keep (list, optional): Message（Message ['glasses', 'left hand']）。
        """
...
class FullBackboneWrapper1D(nn.Module):
    """
    Message window Message backbone，Message：
    - Message backbone Message f: [B,D,Lw]
    - Message stride(frames_per_bin) Message f Message feature Message
    - Message（Message），Message global_feat: [B,D,T_global]
    """
    ...
    def recalibrate(self, device, dtype):
        """
        Message“Message -> Message”Message stride。
        Message CNN/Conv Message：frames_per_bin ≈ delta_raw / delta_L
        """
        T0 = self.win_len
...
def handle_nan_and_interpolate(data, window_len, target_len):
    ...
    Message NaN Message。
    Args:
        data (np.ndarray): Message，Message (window_len, ...)
        window_len (int): Message。
        target_len (int): Message。
    Returns:
        np.ndarray: Message，Message target_len，Message。
    """
