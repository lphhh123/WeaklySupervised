...
class WeaklySupervisedXRFV2DatasetTrain(Dataset):
    ...
    def _load_stats(self, modality='imu'):
        ...
        if modality not in stats:
            raise KeyError(f"global_stats.json does not contain stats for {modality}")
        return (
    ...
    def _get_video_level_label(self, sample_idx):
        """Extract video-level labels from anchor labels (mapped to new IDs)."""
    ...
    def _preprocess_imu(self, sample_30s_np):
        ...
        -> [5, 6, 2048] -> [30, 2048], then normalize with imu global_mean/_std.
        """
    ...
    def _preprocess_airpods(self, sample_air_np):
        ...
        Keep acceleration (3:6) + angular velocity (6:9), total 6 dims:
        -> [2048, 6] -> [6, 2048], then normalize with airpods global_mean/_std.
        """
    ...
    def __getitem__(self, idx):
        ...
        if self._debug_print:
            print("Debug: WeakIMUDatasetTrain sample")
            print(f"idx = {idx}")
