...
def frame_probs_to_segments(probs, fps, threshold=0.1, min_duration=0.2):
    """
    Message，Message threshold Message (Message 0.1-0.2)
    """
    T, C = probs.shape
...
def test_cdur_xrfv2(config, checkpoint_path, test_mode="test_window"):
    """
    Message test_window Message test_full（Message + NMS）
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    print(f"[{test_mode}] Message: {total_preds}")

    if total_preds == 0:
        print(f"Message: {test_mode} Message。")
        return 0.0
