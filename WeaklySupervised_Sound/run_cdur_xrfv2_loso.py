...
def run_train_fold(train_paths, save_dir, device):
    ...
    print(f"\nMessage [Message: {len(combined_train_ds)}]")
    for epoch in range(config["training"]["epochs"]):
        ...
        print(f"Epoch {epoch + 1:03d} Message | Message Loss: {avg_loss:.4f}")
...
def run_dual_test_fold(test_path, checkpoint_path, device, test_mode="test_full"):
    ...
    for file_name, data_iter in tqdm(test_ds.dataset(), desc=f"Message ({test_mode})", leave=False):
...
if __name__ == "__main__":
    ...
    for i, test_path in enumerate(data_paths):
        ...
        print(f"\n" + "=" * 70)
        print(f"FOLD {i + 1}/4 | Message: {sub_id}")
        print(f"=" * 70)
        ...
        for mode in ["test_window", "test_full"]:
            ...
            print(f"--- [{mode}] Message | Message mAP: {avg_mAP:.4f}")


    with open(os.path.join(config["path"]["result_root"], "loso_final_metrics.json"), 'w') as f:
        json.dump(all_metrics, f, indent=4)
    print("\n>>> LOSO Message。Message loso_final_metrics.json。")