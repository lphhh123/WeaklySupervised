...
def _load_gt_segments(gt_path, video_id, subset="test"):
    """
    Compatible with ActivityNet-style GT:
    data["database"][video_id]["annotations"] contains:
      - label: str
      - segment: [t0, t1]  (usually seconds)
    """
...
def _load_pred_segments(pred_path, video_id):
    ...
    data["results"][video_id] contains:
      - label: str
      - score: float
      - segment: [t0, t1]
    """
...
def _build_label2color(labels):
    ...
    """
    Build a richer palette of 60 colors by combining tab20, tab20b, and tab20c.
    """
...
def merge_same_label(segs, gap=0.0):
    """Merge overlapping/adjacent segments of the same label (within gap)."""
    by = defaultdict(list)
...
def suppress_diff_labels_drop(pred_segs):
    """
    If different labels overlap, keep the higher score and drop the lower segment.
    """
...
def plot_action_sequences_frameless(
    ...
    """
    Frameless version: removes outer borders and manually adds sequence names on the left.
    """
...
if all_labels:
    ...
    print(f"No data found to plot for video_id '{video_id}'.")
