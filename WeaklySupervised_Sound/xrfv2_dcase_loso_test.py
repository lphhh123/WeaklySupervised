...
def temporal_nms(predictions, thresh=0.4):
    """Temporal NMS: merge duplicate predictions across windows."""
...
def post_process_for_anet(probs, id_to_label, threshold=0.3, offset_frame=0):
    """Convert model outputs to ANET dict format in frame units."""
...
def run_loso_test():
    ...
    for folder in loso_folders:
        print(f"\nTesting LOSO experiment: {folder}")
        ...
        if not os.path.exists(best_ckpt):
            print(f"Skipping {folder}, checkpoint not found: {best_ckpt}")
            continue
