...
class WeaklySupervisedXRFV2DatasetTrain(Dataset):
    ...
    def _load_stats(self, modality='imu'):
        ...
        if modality not in stats:
            raise KeyError(f"global_stats.json is missing statistics for {modality}")
        return (
    ...
    def _get_video_level_label(self, sample_idx):
        """Extract video-level labels from anchors (mapped to new IDs)."""
    ...
    def _preprocess_airpods(self, sample_air_np):
        ...
        Use only acceleration (3:6) + gyroscope (6:9), total 6 dims:
...
class WWADLDatasetTestSingle():
    ...
    def get_data_full(self, file_path):              
        sample = self.modality_dataset(file_path,
                                       receivers_to_keep=None,
                                       new_mapping=self.new_mapping)
        sample_count = len(sample.data)
...
class WeaklySupervisedXRFV2DatasetTest:
    def __init__(self,
        ...
        assert modality == 'imu', "WeaklySupervisedXRFV2DatasetTest only supports IMU as the main modality"
    ...
    def dataset(self):
        ...
        # ========== 2) IMU + AirPods ==========
            ...
            for (imu_name, imu_iter), (air_name, air_iter) in zip(imu_files, air_files):
                assert imu_name == air_name, f"IMU/AirPods filename mismatch: {imu_name} vs {air_name}"
...
class WWADL_imu(WWADLBase):
    ...
    Data shape notes:
    - data shape: (2900, 5, 6)
        - First dim (2900): sample count (e.g., time steps)
        - Second dim (5): device count (5 IMU devices; see name_to_id)
        - Third dim (6): IMU channels (e.g., accel + gyro axes)
    ...
    def __init__(self, file_path, receivers_to_keep=None, new_mapping=None):
        """
        Initialize IMU data handler and optionally keep specified devices.
        Args:
    ...
    def retain_devices(self, devices_to_keep):
        """
        Filter and keep specified device dimensions.
...
def handle_nan_and_interpolate(data, window_len, target_len):
    """
    Interpolate data and handle NaN values before interpolation.
    Args:
        data (np.ndarray): input data, shape (window_len, ...)
        window_len (int): original sequence length
        target_len (int): target sequence length
    Returns:
        np.ndarray: interpolated data; time dimension becomes target_len while others stay unchanged.
    """
...
class FullBackboneWrapper1D(nn.Module):
    """
    Extend a fixed-window backbone to support full sequences:
    - Slide the window to get segment features f: [B,D,Lw]
    - Use effective stride (frames_per_bin) to place each f back into the global feature sequence
    - Average overlaps to get global_feat: [B,D,T_global]
    """
    ...
    def recalibrate(self, device, dtype):
        """
        Estimate effective stride via length delta: output length change vs input length change.
        Stable for CNN/Conv stacks: frames_per_bin ≈ delta_raw / delta_L
        """
        T0 = self.win_len
