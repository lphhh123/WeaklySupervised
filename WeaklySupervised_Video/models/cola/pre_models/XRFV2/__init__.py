...
def _adapt_time_dependent_weights(state_dict, model):
    """
    Added to fix TSSE size mismatches by interpolating 7s pretrained weights
    (L=239/120) to 30s model sizes (L=1024/512).
    """
    ...
    for k, v in state_dict.items():
        ...
        # 2. ContraNorm LayerNorm: [L_src] -> [L_tgt]
            ...
            print(f"   Warning: skipping mismatched key {k}: {source_shape} vs {target_shape}")
