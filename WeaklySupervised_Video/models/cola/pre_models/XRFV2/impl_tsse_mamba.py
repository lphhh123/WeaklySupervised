...
class TADEmbedding_7s(nn.Module):
    """
    xrfv2 layout: Embedding -> TSSE x layer.
    Length is inferred with a dummy input to avoid stride/ceil pitfalls (e.g., 478).
    """
...
def _crop_pyramid_to_valid(feats, valid_len0: int, scale_factor: int = 2):
    ...
    valid_len0: effective length of level0 (before padding)
    Assume each level downsamples by ~1/scale_factor (consistent with xrfv2/FPNIdentity)
    """
...
class Mamba_config:
    """
    Replicate xrfv2-style Mamba_config without relying on the register system.
    """
...
class Mamba(nn.Module):
    """
    Replicate the xrfv2 Mamba wrapper:
      MambaBackbone + (optional) FPNIdentity
    Forward supports external batched_masks (for padding/mask).
    """
...
class TSSE_MambaBackbone_7s(nn.Module):
    """
    Full xrfv2 layout:
      embedding = Embedding or TADEmbedding_7s
      backbone  = Mamba(config)
    forward:
      x = embedding(x)
      (pad+mask)
      feats = backbone(x, mask)
      return feats[0] (highest resolution)
    """
    def __init__(
        self,
        in_channels: int = 30,
        ...
        super().__init__()
        if feat_dim != 512:
            raise ValueError("xrfv2 TSSE/Mamba channels are fixed to 512; pretraining uses feat_dim=512")
