...
class Actionness_Module(nn.Module):
    def __init__(self, len_feature, num_classes, cfg):
        ...
        self.backbone = backbone_factory(
            dataset_name=getattr(cfg, "DATASET_NAME", "XRFV2"),
    ...
    def extract_features(self, x):
        """
        [New interface] Extract features only, used for test_full feature stitching.
        Input x: [B, T, C] (e.g., [1, 1500, 3])
        Output: [B, 2048, T] (e.g., [1, 2048, 1500])
        """
        T_input = x.shape[1]
    ...
    def predict(self, embeddings):
        """
        [New interface] Classification only, for stitched long feature maps.
        Input embeddings: [B, 2048, T_long]
        """
    ...
    def forward(self, x):
        """
        [Original interface] Keep unchanged, used for training and window inference.
        """
...
class CoLA(nn.Module):
    ...
    def easy_snippets_mining(self, actionness, embeddings, k_easy):
        select_idx = torch.ones_like(actionness, device=actionness.device)
