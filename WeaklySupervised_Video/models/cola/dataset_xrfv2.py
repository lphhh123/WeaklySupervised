...
class XRFV2Dataset(data.Dataset):
    def __init__(self, mode, modal, num_segments, class_dict, cfg, seed=-1, supervision='weak'):
        self.mode = mode
        self.modal = modal
    ...
    def _init_train(self):
        """[Rollback] Initialize training data by loading H5 and all weak labels."""
    ...
    def dataset_windowed(self, clip_length=2048, stride=512):
        """
        Fully aligned with the reference test_window data generation logic.
        Supports 36-dim data (IMU 30 + AirPods 6) concatenation and temporal alignment.
        """
        for file_name in self.file_names:
            ...
            if self.use_airpods:
                ...
                if os.path.exists(air_path):
                    ...
                    print(f"Warning: missing AirPods file; padding with zeros: {air_path}")
