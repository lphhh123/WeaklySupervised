...
def load_label_mapping(mapping_path):
    ...
    - id_to_action: {old_id: action name}
    - old_to_new: {old_id: new_id}
    - new_to_action: {new_id: action name} (added for test-time mapping)
    """
    with open(mapping_path, 'r') as f:
        mapping = json.load(f)

    # Old ID -> action name (keys to int)
    id_to_action = {int(k): v for k, v in mapping["id_to_action"].items()}
    # Old ID -> new ID (keys to int)
    ...
    for old_id, act_name in id_to_action.items():
        ...
        if new_id not in new_to_action:
            new_to_action[new_id] = act_name  # Keep the first mapped action name (group names are similar)
