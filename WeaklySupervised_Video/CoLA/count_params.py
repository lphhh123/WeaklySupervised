...
def count_parameters_in_ckpt(ckpt_path):
    """Load a checkpoint and count parameters."""
    try:
        ...
        if isinstance(checkpoint, dict):
            ...
            print(f"   Unrecognized checkpoint format: {ckpt_path}")
            return None
        ...
        print(f"   Failed to read {ckpt_path}: {e}")
        return None


def get_files_standard(root_dir):
    """
    Standard layout: root/seed_*/fold*/model_final.pth
    """
    files = []
...
def get_files_xrfv2(root_dir):
    """
    XRFV2 layout: root/seed_*/checkpoints/model_best.pth
    """
    files = []
    for pattern in FILENAME_PATTERNS:
...
def main():
    print("Starting model parameter count...")
    results = []
    ...
    print("\n" + table.table)
    print("\nNote: parameter counts are in millions (M), size estimated with float32.")
