...
class BatteryParams:
    ...
    def get_SOH(self, N):
        """Compute capacity retention (SOH) based on the square-root law."""
        return 1.0 - self.beta_sei * np.sqrt(N)

    def get_R0(self, T_kelvin, N):
        """Compute series resistance with temperature (Arrhenius) and aging."""
    ...
    def get_capacity_correction(self, T_celsius):
        """Piecewise cubic spline correction for CF(T)."""
        dT = T_celsius - 25.0
        if T_celsius < 10:
