...
class XRFV2Dataset(data.Dataset):
    ...
    def _init_train(self):
        """[Message] Message：Message H5 Message Message"""
    ...
    def dataset_windowed(self, clip_length=2048, stride=512):
        """
        Message test_window Message。
        Message 36 Message (IMU 30 + AirPods 6) Message。
        """
        for file_name in self.file_names:
            ...
            if not os.path.exists(file_path):
                print(f"⚠️ Message（Message）: {file_path}")
                continue
            ...
            if self.use_airpods:
                ...
                if os.path.exists(air_path):
                    ...
                    print(f"⚠️ Message AirPods Message，Message: {air_path}")
