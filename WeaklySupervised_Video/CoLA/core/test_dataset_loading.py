...
def check_dataset_airpods():
    print("======== Message XRFV2Dataset (IMU + AirPods) ========")
    ...
    print("\n[Step 1] Message Train Mode (H5 Message IMU+AirPods)...")
    try:
    ...
    print("\n[Step 2] Message Test Mode (Message IMU+AirPods)...")
    try:
        ...
        for i in range(len(test_ds)):
            ...
            if target_vid in vid:
                ...
                if data.shape == (2048, 36):
                    print(f"     ✅ Data Shape: {data.shape} (Message!)")
                else:
                ...
                print(f"     IMU Message: {imu_part.abs().sum().item():.2f}")
                print(f"     AirPods Message: {airpods_part.abs().sum().item():.2f}")

                if airpods_part.abs().sum().item() < 1e-5:
                    print(
                        "     ❌ Message: AirPods Message 0！Message /home/lipei/WWADL/AirPodsPro(Message airpods) Message。")
                else:
                    print("     ✅ AirPods Message (Message)。")
