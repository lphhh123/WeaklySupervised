...
RESULTS_ROOT = "./output_wetlab_cola_ddp"
...
def main():
    ...
    print(f"📍 Message {os.path.abspath(RESULTS_ROOT)} Message...")
    for seed in SEEDS:
        seed_dir = os.path.abspath(os.path.join(RESULTS_ROOT, f"seed_{seed}"))
        if not os.path.exists(seed_dir):
            print(f"❌ Message: {seed_dir}")
            continue

        print(f"🚀 Message Seed {seed}: Message LOSO Message...")


        try:
            run_seed_eval(seed_dir)
        except Exception as e:
            print(f"   ⚠️ Seed {seed} Message: {e}")
            continue


        summary_path = os.path.join(seed_dir, "metrics_summary_all_folds.json")
        if not os.path.exists(summary_path):
            print(f"   ⚠️ Message: {summary_path}")
            continue
...
def print_final_table(mode_name, acc):
    ...
    print(f"\n{'=' * 35} {mode_name.upper()} FINAL SUMMARY {'=' * 35}")
    print(f"{'Mean of 3 Seeds (22 Folds each)':^80}")
