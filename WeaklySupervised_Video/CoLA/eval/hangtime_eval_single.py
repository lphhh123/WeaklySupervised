...
def run_optimized_inference():
    ...
    print(f"\nInference completed; JSON saved to: {pred_path}")
    print("Starting automatic evaluation script...")
    ...
    with open(summary_path, 'r') as f:
        ...
        print("\nCorrected Fold0 results")
        print(f"mAP@Avg: {stats['mAP_mean']:.4f}")
        print(f"F1-Macro: {stats['F1_macro']:.4f}")
        print(f"IR (Insertion Ratio): {stats['UODIFM']['IR']:.4f} (expected to drop significantly)")
