...
def run_seed_eval(seed_path):
    """Run the reference script to compute the 4-fold average for this seed."""
...
def main():
    ...
    print(f"Reading results root: {os.path.abspath(RESULTS_ROOT)}")
    ...
    for seed in SEEDS:
        ...
        print(f"Processing Seed {seed}: running 4-fold LOSO summary evaluation...")

                                                    
        try:
            run_seed_eval(seed_dir)
        except Exception as e:
            print(f"   Warning: Seed {seed} evaluation script failed: {e}")
            continue
