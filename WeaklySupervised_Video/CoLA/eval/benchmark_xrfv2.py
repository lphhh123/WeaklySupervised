...
def run_inference():
    ...
    net.eval()
    print(f"Model loaded successfully: {ckpt_path}")
    ...
    print("Evaluating inference speed and GPU memory usage...")
    with torch.no_grad():
    ...
    print(f"Performance data saved to: {perf_file_path}")
    print(f"   [Speed: {perf_results['avg_inference_time_ms']} ms | Memory: {perf_results['peak_gpu_memory_mb']} MB]")

                         
    print("\nContinuing with the standard evaluation pipeline...")
