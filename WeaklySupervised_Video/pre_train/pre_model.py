...
class VGG1DBackboneBUAA(nn.Module):
    """
    在现有 VGG1DBackbone 的结构上，forward 加 BUAA（无参数）。
    输出仍为 [B,512,T_top]，T_top是b5输出长度（约 T/32）。
    """
    ...
    def forward(self, x):
        ...
        def att(feat):
            # 1) resize到 top 长度（对应论文的 “对齐到顶层分辨率”）
        ...
        A = torch.stack([a1, a2, a3, a5], dim=0).amax(dim=0)  # [B,T_top]  element-wise max 聚合
