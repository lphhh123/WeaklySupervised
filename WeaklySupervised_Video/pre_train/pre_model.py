...
class VGG1DBackboneBUAA(nn.Module):
    """
    Message VGG1DBackbone Message，forward Message BUAA（Message）。
    Message [B,512,T_top]，T_topMessageb5Message（Message T/32）。
    """
    ...
    def forward(self, x):
        ...
        A = torch.stack([a1, a2, a3, a5], dim=0).amax(dim=0)
...
class AFBlock1D_7s(nn.Module):
    """
    ActionFormer Message 1D Message：
...
class ActionFormerBackbone_7s(nn.Module):
    """
    Message ActionFormer-style backbone：
      Conv1d (Message) + Message dilated conv block + Transformer (Message)。
      Message: (B, 30, 478)
      Message: (B, feat_dim, 478)
    """
    def __init__(
        self,
        in_channels: int = 30,
...
class PositionalEncoding1D(nn.Module):
    """Message Transformer 1D Message，Message T > max_len Message。"""
...
class TemporalTransformer1D(nn.Module):
    """
    Message nn.TransformerEncoder Message 1D Transformer：
      - Message (B, C, T)，Message (B, T, C) Message self-attn。
    """
    def __init__(
        self,
        d_model: int,
