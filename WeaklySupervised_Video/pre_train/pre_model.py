...
class VGG1DClassifier(nn.Module):
    def __init__(self, num_classes, task='single', in_channels=30, feat_dim=512):
        super().__init__()
        self.task = task
        self.in_channels = in_channels
        self.backbone = VGG1DBackbone(in_channels=in_channels, feat_dim=feat_dim)
...
class VGG1DBackboneBUAA(nn.Module):
    """
    Adds a BUAA branch (no parameters) on top of VGG1DBackbone.
    Output stays [B,512,T_top], where T_top is the b5 output length (~T/32).
    """
    ...
    def forward(self, x):
        ...
        def att(feat):
            # 1) Resize to top length (paper's "align to top-level resolution")
            feat_r = F.adaptive_max_pool1d(feat, output_size=T_top)   # [B,C,T_top]
            # 2) Channel mean -> [B,T_top]
            a = feat_r.mean(dim=1)
            # 3) per-sample min-max -> [B,T_top]
            a = minmax_norm_1d_per_sample(a)
            return a

        # Aggregate x1/x2/x3/x5
        a1 = att(x1)
        a2 = att(x2)
        a3 = att(x3)
        a5 = att(x5)

        A = torch.stack([a1, a2, a3, a5], dim=0).amax(dim=0)  # [B,T_top] element-wise max aggregation
...
# Model 5: LSTM_7s (temporal dependency modeling)
...
# Model 6: Transformer_7s (global context capture)
class TransformerBackbone_7s(nn.Module):
    def __init__(self, in_channels=30, d_model=128, nhead=4, num_layers=3, feat_dim=512):
        ...
        self.pos_encoder = nn.Parameter(torch.randn(1, d_model, 478))  # Fixed positional encoding for 7s windows
...
# Models 1-3: TCN series_7s (temporal dependency + residual + multi-scale)
...
class TCNClassifier_7s(nn.Module):
    ...
    def forward(self, x):
        feat = self.backbone(x)
        return self.classifier(feat)
# Continue filling in the remaining parts of models 1-3 (ResTCN_7s, MS-TCN_7s)
...
class AFBlock1D_7s(nn.Module):
    """
    ActionFormer-style 1D residual convolution block:
...
class ActionFormerBackbone_7s(nn.Module):
    """
    Simplified ActionFormer-style backbone:
      Conv1d (local) + multi-layer dilated conv block + Transformer (global).
      Input: (B, 30, 478)
      Output: (B, feat_dim, 478)
    """
    def __init__(
        self,
        in_channels: int = 30,
        ...
        super().__init__()
        # Map 30 channels to feat_dim
        self.input_proj = nn.Conv1d(in_channels, feat_dim, kernel_size=1, bias=False)

        # Multi-layer dilated residual conv block (local temporal modeling)
        ...
        # Transformer (long-range dependency / inter-segment relations)
        ...
        # Output normalization; GN is recommended for consistency
        self.out_norm = nn.GroupNorm(num_groups=32, num_channels=feat_dim)

    def forward(self, x):
        # x: (B, 30, 478)
        x = self.input_proj(x)   # (B, feat_dim, T)
        x = self.blocks(x)       # (B, feat_dim, T), local convolution modeling
        x = self.trans(x)        # (B, feat_dim, T), self-attention for inter-segment relations
        x = self.out_norm(x)
...
class PositionalEncoding1D(nn.Module):
    """Standard Transformer 1D positional encoding with dynamic generation when T > max_len."""
    def __init__(self, d_model: int, max_len: int = 512):
        super().__init__()
        self.d_model = d_model

        # Pre-generate a max_len segment for short sequences (e.g., 7s with 478 frames)
        ...
        self.register_buffer("pe", pe)  # Buffer stays (1, 512, d_model) for loading legacy checkpoints
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if T <= self.pe.size(1):
            # Use the pre-generated segment for short sequences
            pe = self.pe[:, :T, :]                             # (1, T, C)
        else:
            # For longer sequences (e.g., 30s with 2048 frames), generate positional encoding dynamically
            device = x.device
            dtype = x.dtype
...
class TemporalTransformer1D(nn.Module):
    """
    1D Transformer based on nn.TransformerEncoder:
      - Input (B, C, T); internally transpose to (B, T, C) for self-attention.
    """
    def __init__(
        self,
        d_model: int,
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=True,   # Accepts (B, T, C)
            norm_first=True,    # LayerNorm first for slight stability gain
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x.transpose(1, 2)  # Transpose back to (B, C, T)
