...
def _load_loso_json(json_path: str):
    ...
    Here s,e are frame indices (int), with ranges treated as [s, e).
    """
...
def npy_windows_to_raw_frames(
    ...
    """
    NPY shape: [N, num_sensors*win_samples], flattened in sensor-major order:
      sensor0(30) + sensor1(30) + ... + sensor112(30)

    NPY windows: win_samples(=30) samples, overlap(=0.5) -> stride=15 samples

    Reconstruct continuous 30Hz raw:
      The first window takes all 30 frames.
      Subsequent windows append only the newly appearing frames (the last 15 frames).
...
def _compute_mean_var_across_subjects(
    ...
    """
    Compute mean/var per channel from reconstructed continuous raw (30Hz) sequences.
    """
...
def ensure_all_loso_stats_json(
    ...
    """
    Ensure stats for all 4 folds exist (leave-one-subject-out):
      raw/loso_norm_stats_json/loso_sbj_k_stats.json

    If the stats directory or any fold JSON is missing, compute and write it.
    """
    for k in folds:
...
def load_fold_mean_std_from_json(
    ...
    """
    Read stats JSON for a fold, return mean/std, and validate key metadata to avoid misuse.
    """
