...
def run_loso_pretrain(config: dict):
    ...
    Default: eval_loader uses test_ds directly (explicit train/test split).

    Optional: when val_mode="train_subject", sample one subject from training subjects for validation,
    while test_ds is still reserved for final testing.
    """
    ...
    for fold in range(config.get("loso_number", 1)):
        ...
        print(f"\n[LOSO] Fold {fold} | json={loso_json} | val_mode={val_mode}")
