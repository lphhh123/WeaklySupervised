...
# train one fold
# ============================================================
def train_rskp_one_fold_hangtime(config, fold: int, exp_name: str = "rskp_hangtime", device=None):
    if device is None:
...
# ============================================================
@torch.no_grad()
def test_rskp_hangtime(config, checkpoint_path, fold: int, test_mode: str = "test_window", device=None):
    """
    test_mode:
    ...
    for x, y, meta in tqdm(loader, desc=f"[RSKP Test] fold{fold} {test_mode}"):
        ...
        for k in range(num_classes):
            ...
            scores = ndimage.gaussian_filter1d(scores, sigma=0.5)
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "RSKP-hangtime-v1.0", "results": results, "external_data": {}},
...
# multi-fold runner
# ============================================================
def run_loso_rskp_hangtime(config, device=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for i, fold in enumerate(folds):
        ...
        # 1) train
        rskp_ckpt = train_rskp_one_fold_hangtime(config, fold, exp_name=config.get("exp_name", "rskp_hangtime"), device=device)

        # 2) test_window
        mAPs_w, avg_w, pred_w = test_rskp_hangtime(config, rskp_ckpt, fold=fold, test_mode="test_window", device=device)

        # 3) test_full
        mAPs_f, avg_f, pred_f = test_rskp_hangtime(config, rskp_ckpt, fold=fold, test_mode="test_full", device=device)
...
if __name__ == "__main__":
    ...
    config = {
        "seed": args.seed,
        "exp_name": "rskp_hangtime",

        "dataset_dir": "/home/lipei/TAL_data/hangtime/",
        "pretrained_dir": "/home/lipei/project/WSDDN/OtherData/HANGTIME/pre_train/CNN1D",
        "checkpoint_dir": f"/home/lipei/project/WSDDN/checkpoints/HANGTIME/{args.seed}/rskp_0125",
        "result_root": f"/home/lipei/project/WSDDN/test_results/HANGTIME/{args.seed}/rskp_0125",
    ...
    run_loso_rskp_hangtime(config, device=device)
