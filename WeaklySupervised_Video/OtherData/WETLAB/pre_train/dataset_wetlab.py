...
# ----------------------------
# Dataset (wetlab Pretrain)
# ----------------------------
class WetlabDataset_10s(Dataset):
    """
    Pretraining dataset:
    - Reconstruct continuous 50Hz raw [T,21] from processed/.../sbj_k.npy
    - If the window center frame falls within a GT segment -> label_id
    - Normalization: read raw/<stats_dirname>/loso_sbj_{fold}_stats.json (mean+var)
    """
    def __init__(
        self,
        dataset_dir: str,
    ...
    def __getitem__(self, idx: int):
        ...
        raw = self._get_raw_array(sbj)              # [T,21]
        x = raw[s:e, :]                             # [win_len, 21]
        x = torch.from_numpy(x.T.copy()).float()    # -> [21, win_len]
