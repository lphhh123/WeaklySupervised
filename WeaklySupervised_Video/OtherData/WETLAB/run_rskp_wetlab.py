...
# train one fold
# ============================================================
def train_rskp_one_fold_wetlab(config, fold: int, exp_name: str = "rskp_wetlab", device=None):
    if device is None:
    ...
    pretrain_path = os.path.join(
        ...
        f"wetlab_{config.get('pretrained_model_name','CNN1D')}_pretrained_loso_sbj_{fold}.pth"
    )
    ...
    base_train_ds = WeaklyWetlabDataset(
...
def test_rskp_wetlab(config, checkpoint_path, fold: int, test_mode: str = "test_window", device=None):
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "RSKP-wetlab-v1.0", "results": results, "external_data": {}},
...
# multi-fold runner
# ============================================================
def run_loso_rskp_wetlab(config, device=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for i, fold in enumerate(folds):
        ...
        # 1) train
        rskp_ckpt = train_rskp_one_fold_wetlab(config, fold, exp_name=config.get("exp_name", "rskp_wetlab"), device=device)

        # 2) test_window
        mAPs_w, avg_w, pred_w = test_rskp_wetlab(config, rskp_ckpt, fold=fold, test_mode="test_window", device=device)

        # 3) test_full
        mAPs_f, avg_f, pred_f = test_rskp_wetlab(config, rskp_ckpt, fold=fold, test_mode="test_full", device=device)
...
if __name__ == "__main__":
    ...
    config = {
        "seed": args.seed,
        "exp_name": "rskp_wetlab",

        "dataset_dir": "/home/lipei/TAL_data/wetlab/",
        "pretrained_dir": "/home/lipei/project/WSDDN/OtherData/WETLAB/pre_train/CNN1D",
        "checkpoint_dir": f"/home/lipei/project/WSDDN/checkpoints/WETLAB/{args.seed}/rskp_0125",
        "result_root": f"/home/lipei/project/WSDDN/test_results/WETLAB/{args.seed}/rskp_0125",
    ...
    run_loso_rskp_wetlab(config, device=device)
