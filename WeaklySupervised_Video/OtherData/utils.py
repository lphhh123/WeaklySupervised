...
def _majority_label_in_window(win_s: int, win_e: int, annos, win_len: int, min_frac: float = 0.0):
    """
    Within [win_s, win_e), count overlap frames per label and return the label with the largest fraction.
    - annos: [(s,e,lid), ...] sorted by s, semantics are [s,e)
    - min_frac: if the max fraction < min_frac, return None (treated as background)
      default 0.0: any overlap will assign a label
    """
    if win_e <= win_s:
        return None
...
def load_mean_std_from_meanvar_json(
    ...
    """
    Read the provided JSON format (mean + var) and strictly validate metadata
    to avoid using the wrong fold or mismatched parameters.
    """
...
def _uniform_pick_indices(n: int, k: int):
    """Uniformly pick k indices from a sequence of length n (cover the full range)."""
    if k <= 0 or n <= 0:
        return []
    if k >= n:
        return list(range(n))
...
def build_gt_for_anet(loso_json_path, out_gt_path):
    """
    Convert subset=Validation to test in the LOSO JSON for ANETdetection.
    """
...
def featbox_to_time_seconds(
    ...
    """
    Convert proposal index space ([start_idx,end_idx) in T_global) to seconds.
    """
...
def generate_proposal_boxes(
    ...
    """
    Output proposals in feature index space: Tensor[P,2], each row [start,end).
    Multi-scale fixed windows: scales come from a discrete set in [min_sec, max_sec] (controlled by sec_resolution).
    """
...
class GlobalBackboneWrapper(nn.Module):
    ...
    def forward(self, x_raw: torch.Tensor, return_info: bool = False):
        """
        x_raw: [B, C, T_frames] (typically T_frames=900 during training)
        return:
...
def _to_jsonable(obj):
    """Convert numpy types in config to JSON-serializable Python built-ins."""
