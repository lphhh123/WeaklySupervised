...
# Build mapping from pretrained weight keywords to backbone classes: name -> (class, ckpt, feat_dim, input_length).
PRETRAINED_ZOO = {
    "CNN1D": {
        "cls": CNN1DBackbone,
        "ckpt": None,
...
def _init_backbone_weights(m: nn.Module):
    """
    Generic initialization: Conv/Linear use Kaiming, Norm layers use weight=1 and bias=0.
    """
...
def load_pretrained_backbone(
    ...
    if ckpt_path is not None and os.path.exists(ckpt_path):
        ...
        print(f"[Backbone] Using pretrained weights")
        print(f"[Backbone] name={pretrained_name}")
        print(f"  ckpt={ckpt_path}")
        print(f"  missing={missing}")
        print(f"  unexpected={unexpected}")
    else:
        print(f"[Backbone] Using random initialization (no checkpoint loaded)")
        print(f"[Backbone] name={pretrained_name}")
