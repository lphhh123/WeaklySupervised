...
class Memory(nn.Module):
    ...
    def _sort_permutation(self, inp_mu, inp_sc, idx):
        if inp_sc.dim() == 1:
            ...
            sc_to_concat = inp_sc[:, idx]
        
        device = self.cls_sc_queue.device
        sc_to_concat = sc_to_concat.to(device)
        inp_mu = inp_mu.to(device)
        
        concat_sc = torch.cat([self.cls_sc_queue[idx, ...], sc_to_concat], 0)
