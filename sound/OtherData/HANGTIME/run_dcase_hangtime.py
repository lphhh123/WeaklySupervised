# OtherData/HANGTIME/run_dcase_hangtime.py
# -*- coding: utf-8 -*-
import os
...
# ============================================================
# 2) Train one fold (HANGTIME)
# ============================================================
def train_dcase_one_fold_hangtime(config, fold: int, exp_name: str = "dcase_hangtime"):
    ...
    print(f"\n[Train HANGTIME] fold={fold} | clip={clip_sec}s | samples={len(train_ds)}")
...
# ============================================================
# 3) Test one fold (HANGTIME)
# ============================================================
@torch.no_grad()
def test_dcase_hangtime(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
# 4) Main Runner
# ============================================================
def run_loso_dcase_hangtime(config):
    ...
    clip_sec = config.get("clip_sec", 3.0)
    suffix = f"_{int(clip_sec)}s"
    ...
    for fold in folds:
        # 1. Train
        ckpt = train_dcase_one_fold_hangtime(config, fold, exp_name=config["exp_name"])

        # 2. Test Window
        mAPs_win, avg_mAP_win, th_win = test_dcase_hangtime(config, ckpt, fold, test_mode="test_window")

        # 3. Test Full
        mAPs_full, avg_mAP_full, th_full = test_dcase_hangtime(config, ckpt, fold, test_mode="test_full")
...
if __name__ == "__main__":
    ...
    config = {
        "seed": 2026,
        "exp_name": "dcase_hangtime",
        "model_type": "CRNN",

        "dataset_dir": "/home/lipei/TAL_data/hangtime/",
        "checkpoint_dir": "/home/yinjiaxi/wstal/WeaklySupervised-master/checkpoints/hangtime_dcase_10_5s_2026",
        "result_root": "/home/yinjiaxi/wstal/WeaklySupervised-master/result/hangtime_dcase_10_5s_2026/",
    ...
    run_loso_dcase_hangtime(config)