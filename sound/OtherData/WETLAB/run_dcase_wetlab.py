# OtherData/WETLAB/run_dcase_wetlab.py
# -*- coding: utf-8 -*-
import torch
...
# ============================================================
# 2) Train one fold (Wetlab)
# ============================================================
def train_dcase_one_fold_wetlab(config, fold: int, exp_name: str = "dcase_wetlab"):
    ...
    print(f"\n[Train Wetlab] fold={fold} | clip={clip_sec}s | samples={len(train_ds)}")
...
# ============================================================
# 3) Test one fold (Wetlab)
# ============================================================
@torch.no_grad()
def test_dcase_wetlab(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
# 4) Main Runner
# ============================================================
def run_loso_dcase_wetlab(config):
    ...
    clip_sec = config.get("clip_sec", 10.0)
    suffix = f"_{int(clip_sec)}s"
    ...
    for fold in folds:
        # 1. Train
        ckpt = train_dcase_one_fold_wetlab(config, fold, exp_name=config["exp_name"])

        # 2. Test Window
        mAPs_win, avg_mAP_win, th_win = test_dcase_wetlab(config, ckpt, fold, test_mode="test_window")

        # 3. Test Full
        mAPs_full, avg_mAP_full, th_full = test_dcase_wetlab(config, ckpt, fold, test_mode="test_full")
