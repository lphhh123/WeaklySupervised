...
# ============================================================
# 4) test one fold (Modified)
# ============================================================
@torch.no_grad()
def test_pcl_oicr_opportunity(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for x, y, meta in tqdm(loader, desc=f"[Test PCL/OICR] fold{fold} {test_mode}"):
        ...
        refine_scores = out["refine_scores"]
        final_prob = refine_scores[-1][0, :, 1:]
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "PCL-OICR", "results": results, "external_data": {}},
...
def run_loso_pcl_oicr_opportunity(config):
    ...
    tious = np.linspace(0.3, 0.7, 5)  # 0.3, 0.4, 0.5, 0.6, 0.7
    ...
    for i, fold in enumerate(folds):
        ...
        ckpt = train_pcl_oicr_one_fold_opportunity(
    ...
    for mode_name, metrics_list in all_folds_metrics.items():
        if len(metrics_list) == 0:
            continue

        # stack: [NumFolds, 5] -> mean -> [5]
        stacked_mAPs = np.vstack(metrics_list)
        mean_mAPs = np.mean(stacked_mAPs, axis=0)
        final_avg_mAP = np.mean(mean_mAPs)

        print(f"\nMode: {mode_name}")
        print("-" * 30)
        for i, tiou in enumerate(tious):
            print(f"  tIoU {tiou:.2f} : mAP = {mean_mAPs[i]:.4f}")
        print("-" * 30)
        print(f"  AVG  mAP  :       {final_avg_mAP:.4f}")
...
if __name__ == "__main__":
    ...
    run_loso_pcl_oicr_opportunity(config)