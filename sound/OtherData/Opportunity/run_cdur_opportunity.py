# OtherData/Opportunity/run_cdur_opportunity.py
# -*- coding: utf-8 -*-
...
# 2) Train one fold
# ============================================================
def train_cdur_one_fold_opportunity(config, fold: int, exp_name: str = "cdur_opportunity"):
    ...
    pool_type = config["training"].get("pool_type", "soft")  # Opportunity config recommends soft
    ...
    print(f"\n[Train CDur Opportunity] fold={fold} | samples={len(train_ds)}")
...
# ============================================================
@torch.no_grad()
def test_cdur_opportunity(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    # Load Model
    model_type = config.get("model_type", "CDur")
    pool_type = config["training"].get("pool_type", "soft")

    if model_type == "CDur":
        model = CDur(inputdim=in_channels, outputdim=num_classes, temppool=pool_type)
    else:
        model = MilSEDCNN(inputdim=in_channels, outputdim=num_classes, temppool=pool_type)
    ...
    # Batch size 1 for test usually safer for memory and variable lengths
    ...
    for x, _, meta in tqdm(loader, desc=f"[{test_mode}] fold{fold}", leave=False):
        ...
        segments_per_class = frame_probs_to_segments(frame_prob, fps, threshold=default_threshold)
    ...
    final_results = {"version": "CDur-Opportunity-v1", "results": results_cache, "external_data": {}}
...
# 4) Main Runner
# ============================================================
def run_loso_cdur_opportunity(config):
    ...
    print(f"Starting Opportunity Experiment: {config['exp_name']}")
...
if __name__ == "__main__":
    config = {
        "seed": 2026,
        "exp_name": "cdur_opportunity",
        "model_type": "CDur",

        "dataset_dir": "/home/lipei/TAL_data/opportunity/",
        "checkpoint_dir": "/home/yinjiaxi/wstal/WeaklySupervised-master/checkpoints/opportunity_cdur_10_3s_2026",
        "result_root": "/home/yinjiaxi/wstal/WeaklySupervised-master/result/opportunity_cdur_10_3s_2026",
    ...
    run_loso_cdur_opportunity(config)