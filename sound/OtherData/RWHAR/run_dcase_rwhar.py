# OtherData/RWHAR/run_dcase_rwhar.py
# -*- coding: utf-8 -*-
import torch
...
# ============================================================
# 2) Train one fold (RWHAR)
# ============================================================
def train_dcase_one_fold_rwhar(config, fold: int, exp_name: str = "dcase_rwhar"):
    ...
    print(f"\n[Train RWHAR] fold={fold} | clip={clip_sec}s | samples={len(train_ds)}")
...
# 4) Main Runner
# ============================================================
def run_loso_dcase_rwhar(config):
    ...
    for fold in folds:
        # 1. Train
        ckpt = train_dcase_one_fold_rwhar(config, fold, exp_name=config["exp_name"])

        # 2. Test Window
        mAPs_win, avg_mAP_win, th_win = test_dcase_rwhar(config, ckpt, fold, test_mode="test_window")

        # 3. Test Full
        mAPs_full, avg_mAP_full, th_full = test_dcase_rwhar(config, ckpt, fold, test_mode="test_full")
