...
# Mapping between pretrained weight keywords and backbone classes: name -> (class, ckpt, feat_dim, input_length)
PRETRAINED_ZOO = {
    "CNN1D": {
        "cls": CNN1DBackbone,
...
def _init_backbone_weights(m: nn.Module):
    """
    Generic initialization: Conv/Linear use kaiming; Norm layers set weight=1, bias=0.
    """
...
def load_pretrained_backbone(pretrained_name: str, in_channels: int = 30, device=None):
    ...
    if ckpt_path is not None and os.path.exists(ckpt_path):
        ...
        print(f"[Backbone] Using pretrained model")
