...
class VGG1DBackboneBUAA(nn.Module):
    """
     VGG1DBackbone ，forward  BUAA（parameters）。
    output [B,512,T_top]，T_topb5outputlength（ T/32）。
    """
...
class ActionFormerBackbone_7s(nn.Module):
    """
     ActionFormer-style backbone：
      Conv1d () +  dilated conv block + Transformer (global)。
      input: (B, 30, 478)
...
class PositionalEncoding1D(nn.Module):
    """ Transformer 1D position， T > max_len 。"""
...
class TemporalTransformer1D(nn.Module):
    """
     nn.TransformerEncoder  1D Transformer：
      - input (B, C, T)， (B, T, C)  self-attn。
    """
    def __init__(
        self,
        d_model: int,
