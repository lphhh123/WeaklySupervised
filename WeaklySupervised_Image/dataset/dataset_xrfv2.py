...
class WeaklySupervisedXRFV2DatasetTrain(Dataset):
    ...
    def _load_stats(self, modality='imu'):
        ...
        if modality not in stats:
            raise KeyError(f"global_stats.json  {modality} ")
        return (
...
class WeaklySupervisedXRFV2DatasetTest:
    def __init__(self,
        ...
        assert modality == 'imu', "WeaklySupervisedXRFV2DatasetTest  imu "
    ...
    def dataset(self):
        ...
        # ========== 2) IMU + AirPods ==========
            ...
            for (imu_name, imu_iter), (air_name, air_iter) in zip(imu_files, air_files):
                assert imu_name == air_name, f"IMU/AirPods file: {imu_name} vs {air_name}"
...
class WWADL_imu(WWADLBase):
    ...
    dimension:
    -  shape: (2900, 5, 6)
        - dimension (2900): count（timetime）
        - dimension (5): devicecount（5IMUdevice，position name_to_id）
        - dimension (6): IMUdimension（6）

    name_to_id mappingdeviceposition:
        - 'glasses': 0
        - 'left hand': 1
        - 'right hand': 2
        - 'left pocket': 3
    ...
    def __init__(self, file_path, receivers_to_keep=None, new_mapping=None):
        ...
        Args:
            ...
            devices_to_keep (list, optional): device（ ['glasses', 'left hand']）。
        """
...
def handle_nan_and_interpolate(data, window_len, target_len):
    """
     NaN 。
    Args:
        data (np.ndarray): input，dimension (window_len, ...)
        window_len (int): length。
        target_len (int): length。
    Returns:
        np.ndarray: ，timedimension target_len，dimension。
    """
...
class FullBackboneWrapper1D(nn.Module):
    """
    fixedlength window  backbone，：
    - sliding window backbone feature f: [B,D,Lw]
    -  stride(frames_per_bin)  f global feature 
    - overlapaverage（）， global_feat: [B,D,T_global]
    """
    ...
    def recalibrate(self, device, dtype):
        """
        “length -> outputlength” stride。
         CNN/Conv ：frames_per_bin ≈ delta_raw / delta_L
        """
        T0 = self.win_len
