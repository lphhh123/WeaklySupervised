...
class WSDDN(nn.Module):
    ...
    def forward(self, x, proposal_boxes):
        ...
        proposal_boxes: [B, P, 2]  —  proposal  [start, end]（time）
        """
        B, C, T_global = x.shape
        P = proposal_boxes.shape[1]
...
class TemporalSPP1D(nn.Module):
    ...
    @property
    def out_mul(self):
        """outputdimension C * out_mul （out_mul = sum(levels)）"""
...
class WSDDNTransformerIMU(nn.Module):
    """
    WSDDN + Transformer （proposal  1D SPP）：
    -  proposal  backbone feature [1, C, T_p]
    -  TemporalSPP1D  1D SPP -> [1, C * sum(levels)]
    -  proposal  token：[B, P, spp_out_dim] ->  d_model
    - time + timeposition -> TransformerEncoder
    - ， WSDDN  class / det 
    - return proposal_ctx  feat_fc7，spatialregularization
    """
    def __init__(
        self,
        num_classes: int = 30,
    ...
    def _pool_proposals(self, x, proposal_boxes):
        """
         backbone feature x ， proposal_boxes  1D SPP。
        x: [B, C, T_global]
    ...
    def forward(self, x, proposal_boxes):
        """
        x: [B, C, T_global]  —— pretraining backbone output
        proposal_boxes: [B, P, 2] —— (start, end)，time（：）
        """
        B, C, T_global = x.shape
        _, P, _ = proposal_boxes.shape
...
def generate_proposal_boxes(
    ...
    """
    outputfeaturespatial proposals: Tensor[P,2]， [start,end)。
    multi-scalefixedwindow： [min_sec, max_sec] （sec_resolution ）
    """
...
def generate_proposals(imu_data, num_proposals=50, base_len=476, step=256, min_len=340, max_len=884):
    """
    :param imu_data: 30s IMU [30, 2048]
    :param base_len: 7sdimension（50Hz×7s=350 → 2048*(7/30)≈476）
    :param step: （256）
    :param min_len/max_len: randomlength
    :return: 50proposal [50, 30, T_proposal]
    """
...
def merge_overlapping_proposals(proposals, det_scores, overlap_thresh=0.5):
    """
    :param proposals: proposal [50, 30, 884]
    :param det_scores: proposalconfidence [50, num_classes]
    :return: merge25proposal [25, 30, 884]
    """
