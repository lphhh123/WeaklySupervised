...
class IMU_PCL_OICR(nn.Module):
    """
    IMU Message PCL/OICR Message（Message 1D SPP）：
      Message: global_feat [B, C, T] Message [B, T, C], proposal_boxes [B, P, 2], labels [B, C]
      Message: Message losses + Message score
      Message: Message mil_score / refine_scores
    """
    def __init__(self,
                 feat_dim,
                 num_classes,
    ...
    def forward(self, global_feat, proposal_boxes, labels=None):
        ...
        # 3) MIL + refine
        mil_score = self.mil_head(x)
        refine_scores_flat = self.refine_head(x)
...
def _get_top_ranking_indices(probs, num_clusters=3, rng_seed=1):
    """
    Message PCL Message k-means Message：
      probs: [P]，Message proposal Message
...
def _get_graph_centers_1d(boxes, cls_prob, im_labels,graph_iou_thresh=0.5,max_pc_num=3,num_kmeans_cluster=3,rng_seed=1):
    """
    PCL Message graph center Message pseudo GT Message，1D Message。
    boxes: [P,2]
    cls_prob: [P,C]
...
def _get_highest_score_proposals_1d(boxes, cls_prob, im_labels):
    """
    OICR Message proposal Message pseudo GT Message，1D Message。
    """
    num_imgs, num_classes = im_labels.shape
    assert num_imgs == 1
    im_labels = im_labels[0].copy()
...
def _assign_clusters_1d(all_segments, proposals, im_labels,
    ...
    """
    Message get_proposal_clusters Message：
      Message labels, cls_loss_weights, gt_assignment
    """
...
def pcl_1d(
    ...
    """
    PCL Message 1D Message：Message + proposal cluster。
    """
...
def oicr_1d(
    ...
    """
    OICR Message 1D Message：Message proposal Message pseudo GT。
    """
    cls_prob = cls_prob.copy()
    if cls_prob.shape[1] != im_labels.shape[1]:
        cls_prob = cls_prob[:, 1:]
