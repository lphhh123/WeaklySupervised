...
# 3) train one fold
# ============================================================
def train_oicrBUAA_one_fold_sbhar(config, fold: int, exp_name: str = "pcl_oicr_sbhar"):
    ...
    pretrain_path = os.path.join(
        ...
        f"sbhar_VGG1D_pretrained_loso_sbj_{fold}.pth"
    )
...
# ============================================================
@torch.no_grad()
def test_oicrBUAA_sbhar(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "OICRBUAA-sbhar-v1.0", "results": results, "external_data": {}},
...
# 5) multi-fold runner
# ============================================================
def run_loso_oicrBUAA_sbhar(config):
    ...
    for i, fold in enumerate(folds):
        ...
        ckpt = train_oicrBUAA_one_fold_sbhar(
            config, fold, exp_name=config.get("exp_name", "pcl_oicr_sbhar")
        )

        mAPs_w, avg_w, pred_w = test_oicrBUAA_sbhar(config, ckpt, fold=fold, test_mode="test_window")
        mAPs_f, avg_f, pred_f = test_oicrBUAA_sbhar(config, ckpt, fold=fold, test_mode="test_full")
...
def run_entry(config):
    return run_loso_oicrBUAA_sbhar(config)


def _parse_args():
    parser = argparse.ArgumentParser(description="Runner for run_oicrBUAA_sbhar")
    parser.add_argument("--config", default="configs/sbhar_oicr.json", help="Path to JSON config")
