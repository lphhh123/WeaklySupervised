...
class WeaklySBHARDataset(Dataset):
    """
    weakly supervised：return raw clip + clip multi-hot label
    Return:
      x: FloatTensor [C, T]
    ...
    def __init__(
        ...
        clip_sec: float = 1000.0,     # SBHAR：1000s clip
        ...
        if self.normalize:
            ...
            if self.mean.shape[0] != self.num_sensors:
                raise RuntimeError(f"mean dim={self.mean.shape[0]} != num_sensors={self.num_sensors}")
            if self.std.shape[0] != self.num_sensors:
                raise RuntimeError(f"std dim={self.std.shape[0]} != num_sensors={self.num_sensors}")
        else:
        ...
        if len(self.index) == 0:
            raise RuntimeError(f"No samples built for mode={mode}. Check clip_sec/overlap and data length.")
