...
def _majority_label_in_window(win_s: int, win_e: int, annos, win_len: int, min_frac: float = 0.0):
    """
    window [win_s, win_e)  label windowoverlap， label。
    - annos: [(s,e,lid), ...]  s ， [s,e)
    - min_frac:  < min_frac，return None（background）
      default 0.0：overlap label
    """
    if win_e <= win_s:
        return None
...
def build_gt_for_anet(loso_json_path, out_gt_path):
    """
     loso json  subset=Validation  test， ANETdetection 
    """
...
def featbox_to_time_seconds(
    ...
    """
    proposal spatial([start_idx,end_idx) in T_global) -> 
    """
...
class GlobalBackboneWrapper(nn.Module):
    ...
    def forward(self, x_raw: torch.Tensor, return_info: bool = False):
        """
        x_raw: [B, C, T_frames] (train T_frames=900)
        return:
