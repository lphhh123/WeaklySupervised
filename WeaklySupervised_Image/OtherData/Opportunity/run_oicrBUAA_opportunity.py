...
def train_pcl_oicr_one_fold_opportunity(config, fold: int, exp_name: str = "pcl_oicr_opportunity"):
    ...
    pretrain_path = os.path.join(
        ...
        f"opportunity_VGG1D_pretrained_loso_sbj_{fold}.pth"
    )
...
def test_pcl_oicr_opportunity(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "OICRBUAA-Opportunity-v1.0", "results": results, "external_data": {}},
...
def run_entry(config):
    return run_loso_pcl_oicr_opportunity(config)


def _parse_args():
    parser = argparse.ArgumentParser(description="Runner for run_oicrBUAA_opportunity")
    parser.add_argument("--config", default="configs/opportunity_oicr.json", help="Path to JSON config")
