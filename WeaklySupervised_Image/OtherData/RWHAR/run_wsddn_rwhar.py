...
from OtherData.utils import _meta_get,_uniform_pick_indices
...
#  train one fold
# ============================================================
def train_wsddn_one_fold(config, fold: int, exp_name: str = "wsddn_opportunity"):
    ...
    pretrain_path = os.path.join(
        ...
        f"rwhar_{config.get('pretrained_model_name','CNN1D')}_pretrained_loso_sbj_{fold}.pth"
    )
    ...
    base_train_ds = WeaklyRWHARDataset(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (sample, proposal_boxes, labels) in enumerate(pbar):
            sample = sample.to(device)         # [B,C,900]
            ...
            global_feat = pretrained_backbone(sample)  # [B,512,T_global]
...
# ============================================================
@torch.no_grad()
def test_wsddn_one_fold(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    """
    test_mode:
      - "test_window": test clip_sec sliding window（training）
      - "test_full"  : （ dataset  mode）
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def run_loso_wsddn(config):
    ...
    for i, fold in enumerate(folds):
        ...
        # 1) train
        wsddn_ckpt = train_wsddn_one_fold(config, fold, exp_name=config.get("exp_name", "wsddn_opportunity"))

        # 2) test_window
        mAPs_w, avg_w, pred_w = test_wsddn_one_fold(config, wsddn_ckpt, fold=fold, test_mode="test_window")

        mAPs_f, avg_f, pred_f = test_wsddn_one_fold(config, wsddn_ckpt, fold=fold, test_mode="test_full")
...
if __name__ == "__main__":
    config = {
        ...
        "checkpoint_dir": "/home/lipei/project/WSDDN/checkpoints/RWHAR/2026/wsddn_01",
        "result_root": "/home/lipei/project/WSDDN/test_results/RWHAR/2026/wsddn_01",
