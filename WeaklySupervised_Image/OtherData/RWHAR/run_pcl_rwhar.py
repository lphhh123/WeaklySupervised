...
# 3) train one fold
# ============================================================
def train_pcl_oicr_one_fold_rwhar(config, fold: int, exp_name: str = "pcl_oicr_rwhar"):
...
# ============================================================
@torch.no_grad()
def test_pcl_oicr_rwhar(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "PCL-OICR-rwhar-v1.0", "results": results, "external_data": {}},
...
def run_loso_pcl_oicr_rwhar(config):
    ...
    for i, fold in enumerate(folds):
        ...
        ckpt = train_pcl_oicr_one_fold_rwhar(
            config, fold, exp_name=config.get("exp_name", "pcl_oicr_opportunity")
        )

        mAPs_w, avg_w, pred_w = test_pcl_oicr_rwhar(config, ckpt, fold=fold, test_mode="test_window")
        mAPs_f, avg_f, pred_f = test_pcl_oicr_rwhar(config, ckpt, fold=fold, test_mode="test_full")
...
def run_entry(config):
    return run_loso_pcl_oicr_rwhar(config)


def _parse_args():
    parser = argparse.ArgumentParser(description="Runner for run_pcl_rwhar")
    parser.add_argument("--config", default="configs/rwhar_pcl.json", help="Path to JSON config")
