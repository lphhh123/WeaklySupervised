...
# train one fold
# ============================================================
def train_rskp_one_fold_wear(config, fold: int, exp_name: str = "rskp_wear", device=None):
    if device is None:
    ...
    base_train_ds = WeaklyWearDataset(
...
# ============================================================
@torch.no_grad()
def test_rskp_wear(config, checkpoint_path, fold: int, test_mode: str = "test_window", device=None):
    """
    test_mode:
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "RSKP-wear-v1.0", "results": results, "external_data": {}},
...
# multi-fold runner
# ============================================================
def run_loso_rskp_wear(config, device=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for i, fold in enumerate(folds):
        ...
        # 1) train
        rskp_ckpt = train_rskp_one_fold_wear(config, fold, exp_name=config.get("exp_name", "rskp_wear"), device=device)

        # 2) test_window
        mAPs_w, avg_w, pred_w = test_rskp_wear(config, rskp_ckpt, fold=fold, test_mode="test_window", device=device)

        # 3) test_full
        mAPs_f, avg_f, pred_f = test_rskp_wear(config, rskp_ckpt, fold=fold, test_mode="test_full", device=device)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "pretrained_dir": "/home/lipei/project/WSDDN/OtherData/WEAR/pre_train/CNN1D",
        "checkpoint_dir": f"/home/lipei/project/WSDDN/checkpoints/WEAR/{args.seed}/rskp_0126",
        "result_root": f"/home/lipei/project/WSDDN/test_results/WEAR/{args.seed}/rskp_0126",
    ...
    run_loso_rskp_wear(config, device=device)
