...
# train one fold
# ============================================================
def train_wsddn_one_fold_hangtime(config, fold: int, exp_name: str = "wsddn_opportunity"):
    ...
    optimizer = optim.Adam(model.parameters(), lr=float(config["training"]["lr"]), weight_decay=1e-6)
...
# ============================================================
@torch.no_grad()
def test_wsddn_hangtime(config, checkpoint_path, fold: int, test_mode: str = "test_window"):
    """
    test_mode:
    ...
    with open(pred_path, "w", encoding="utf-8") as f:
        json.dump({"version": "WSDDN-hangtime-v1.0", "results": results, "external_data": {}},
    ...
    extra_report_path = os.path.join(fold_dir, f"extra_metrics_report_{test_mode}.txt")
    print(f"[Saved] {extra_report_path}")
...
def run_loso_wsddn_hangtime(config):
    ...
    for i, fold in enumerate(folds):
        ...
        # 1) train
        wsddn_ckpt = train_wsddn_one_fold_hangtime(config, fold, exp_name=config.get("exp_name", "wsddn_opportunity"))

        # 2) test_window
        mAPs_w, avg_w, pred_w = test_wsddn_hangtime(config, wsddn_ckpt, fold=fold, test_mode="test_window")

        mAPs_f, avg_f, pred_f = test_wsddn_hangtime(config, wsddn_ckpt, fold=fold, test_mode="test_full")
...
def run_entry(config):
    return run_loso_wsddn_hangtime(config)


def _parse_args():
    parser = argparse.ArgumentParser(description="Runner for run_wsddn_hangtime")
    parser.add_argument("--config", default="configs/hangtime_wsddn.json", help="Path to JSON config")
