...
def test_wsddn_imu(config, checkpoint_path,test_mode: str = "test_window"):
    ...
    if "backbone_state_dict" in checkpoint:
        ...
        print("[Test] MessagebackboneMessage（Messagecheckpoint）")
        print(f"  missing={missing}")
        print(f"  unexpected={unexpected}")
    else:
        print("[Test] MessagebackboneMessage（MessagePRETRAINED_ZOO ckpt）")
    ...
    print(f"MessageT_global：{T_global}")
    ...
    for file_name, data_iterator in tqdm(test_files, desc="Message", unit="Message", total=num_test_files):
    ...
    stats_data = {
        "Message": num_test_files,
        "Message(ms)": f"{avg_inf_time:.2f} ± {std_inf_time:.2f}",
        "MessageGPUMessage(MB)": f"{avg_gpu_mem:.2f} ± {std_gpu_mem:.2f}",
        "Message": conf_thresh,
        "Soft-NMS sigma": nms_sigma,
        "Message": num_proposals
    }
    ...
    print(f"Message：{pred_save_path}")
    ...
    report_content = [
        "=" * 60,
        "WSDDN-IMU Message+Message",
        "=" * 60,
        f"Message：",
        f"  - Message：{config['path']['train_dataset_path']}",
        f"  - Message：{config['training']['num_epochs']}",
        f"  - Message：{config['training']['batch_size']}",
        f"  - Message：{config['training']['num_proposals']}",
        f"  - Message：{checkpoint_path}",
        f"",
        f"Message：",
        f"  - Message：{config['path']['test_dataset_path']}",
        f"  - Message：{num_proposals}",
        f"  - Message：{conf_thresh}",
        f"  - Soft-NMS sigma：{nms_sigma}",
        f"  - Message：{num_test_files}",
        f"",
        f"Message：",
        f"  - Message：{avg_inf_time:.2f} ± {std_inf_time:.2f} ms",
        f"  - MessageGPUMessage：{avg_gpu_mem:.2f} ± {std_gpu_mem:.2f} MB",
        f"",
        f"Message（tIoU=0.3~0.7）：",
        "-" * 60
    ]
    ...
    print("\n" + "\n".join(report_content))
    print(f"\nMessage！Message：{result_path}")


def test_pcl_imu(config, checkpoint_path,test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n[PCL/OICR] Message，Message：{device}")
    print(f"[PCL/OICR] Message：{checkpoint_path}")
    ...
    if "backbone_state_dict" in checkpoint:
        missing, unexpected = pretrained_backbone.load_state_dict(checkpoint["backbone_state_dict"], strict=False)
        print("[PCL/OICR-Test] MessagebackboneMessage（Messagecheckpoint）")
        print(f"  missing={missing}")
        print(f"  unexpected={unexpected}")
    else:
        print("[PCL/OICR-Test] MessagebackboneMessage（MessagePRETRAINED_ZOO ckpt）")
    ...
    print(f"[PCL/OICR] Message T_global：{T_global}")
    ...
    for file_name, data_iterator in tqdm(test_files, desc="[PCL/OICR] Message", unit="Message", total=num_test_files):
        class_outputs = [[] for _ in range(num_classes)]
    ...
    with open(pred_save_path, "w", encoding="utf-8") as f:
        json.dump(prediction_data, f, indent=2, ensure_ascii=False)
    print(f"[PCL/OICR] Message：{pred_save_path}")
    ...
    report_content = [
        "=" * 60,
        "PCL/OICR-IMU Message+Message",
        "=" * 60,
        f"Message：use_airpods={use_airpods}, in_channels={in_channels}",
        f"Message：{num_test_files}",
        "-" * 60,
    ]
    for tiou, mAP in zip(tious, mAPs):
        report_content.append(f"tIoU={tiou:.2f} → mAP={mAP:.4f}")
    report_content.append(f"MessagemAP：{avg_mAP:.4f}")
    ...
    print("\n" + "\n".join(report_content))
    print(f"\n[PCL/OICR] Message！Message：{result_path}")
