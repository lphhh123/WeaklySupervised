...
def test_wsddn_imu(config, checkpoint_path,test_mode: str = "test_window"):
    ...
    if "backbone_state_dict" in checkpoint:
        ...
        print("[Test] trainingbackboneweight（checkpoint）")
        print(f"  missing={missing}")
        print(f"  unexpected={unexpected}")
    else:
        print("[Test] pretrainingbackboneweight（PRETRAINED_ZOO ckpt）")
    ...
    print(f"globalfeaturelengthT_global：{T_global}")
    ...
    for file_name, data_iterator in tqdm(test_files, desc="test", unit="file", total=num_test_files):
    ...
    stats_data = {
        "testfiletotal": num_test_files,
        "averagewindowinferencetime(ms)": f"{avg_inf_time:.2f} ± {std_inf_time:.2f}",
        "averageGPU(MB)": f"{avg_gpu_mem:.2f} ± {std_gpu_mem:.2f}",
        "confidencethreshold": conf_thresh,
        "Soft-NMS sigma": nms_sigma,
        "testproposal": num_proposals
    }
    ...
    print(f"resultssave：{pred_save_path}")

    print("\nstartevaluationactionperformance...")
    ...
    report_content = [
        "=" * 60,
        "WSDDN-IMU training+testsummaryreport",
        "=" * 60,
        f"trainingconfig：",
        f"  - trainingdataset：{config['path']['train_dataset_path']}",
        f"  - training：{config['training']['num_epochs']}",
        f"  - batch：{config['training']['batch_size']}",
        f"  - trainingproposal：{config['training']['num_proposals']}",
        f"  - bestmodelpath：{checkpoint_path}",
        f"",
        f"testconfig：",
        f"  - testdataset：{config['path']['test_dataset_path']}",
        f"  - testproposal：{num_proposals}",
        f"  - confidencethreshold：{conf_thresh}",
        f"  - Soft-NMS sigma：{nms_sigma}",
        f"  - testfiletotal：{num_test_files}",
        f"",
        f"inferenceperformance：",
        f"  - averagewindowinferencetime：{avg_inf_time:.2f} ± {std_inf_time:.2f} ms",
        f"  - averageGPU：{avg_gpu_mem:.2f} ± {std_gpu_mem:.2f} MB",
        f"",
        f"actionevaluationresults（tIoU=0.3~0.7）：",
        "-" * 60
    ]
    ...
    print("\n" + "\n".join(report_content))
    print(f"\ntestcomplete！resultssave：{result_path}")


def test_pcl_imu(config, checkpoint_path,test_mode: str = "test_window"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n[PCL/OICR] starttest，testdevice：{device}")
    print(f"[PCL/OICR] model：{checkpoint_path}")
    ...
    if "backbone_state_dict" in checkpoint:
        missing, unexpected = pretrained_backbone.load_state_dict(checkpoint["backbone_state_dict"], strict=False)
        print("[PCL/OICR-Test] trainingbackboneweight（checkpoint）")
        print(f"  missing={missing}")
        print(f"  unexpected={unexpected}")
    else:
        print("[PCL/OICR-Test] pretrainingbackboneweight（PRETRAINED_ZOO ckpt）")
    ...
    print(f"[PCL/OICR] globalfeaturelength T_global：{T_global}")
    ...
    for file_name, data_iterator in tqdm(test_files, desc="[PCL/OICR] test", unit="file", total=num_test_files):
        class_outputs = [[] for _ in range(num_classes)]
    ...
    with open(pred_save_path, "w", encoding="utf-8") as f:
        json.dump(prediction_data, f, indent=2, ensure_ascii=False)
    print(f"[PCL/OICR] resultssave：{pred_save_path}")

    # 7) mAP
    print("\n[PCL/OICR] startevaluationactionperformance...")
    tious = np.linspace(0.3, 0.7, 5)
    anet_evaluator = ANETdetection(
        ground_truth_filename=test_dataset.eval_gt,
        prediction_filename=pred_save_path,
    ...
    report_content = [
        "=" * 60,
        "PCL/OICR-IMU training+testsummaryreport",
        "=" * 60,
        f"testconfig：use_airpods={use_airpods}, in_channels={in_channels}",
        f"testfiletotal：{num_test_files}",
        "-" * 60,
    ]
    for tiou, mAP in zip(tious, mAPs):
        report_content.append(f"tIoU={tiou:.2f} → mAP={mAP:.4f}")
    report_content.append(f"averagemAP：{avg_mAP:.4f}")
    ...
    print("\n" + "\n".join(report_content))
    print(f"\n[PCL/OICR] testcomplete！resultssave：{result_path}")
