...
def train_wsddn_imu(config, exp_name="wsddn"):
    ...
    print(f"training：{len(train_dataset)}，batch：{config['training']['batch_size']}")

    train_backbone = bool(config["training"].get("train_backbone", False))
    if not backbone_loaded:
        train_backbone = True
        print("[Train] pretraining ->  train_backbone=True（randomtraining）")
    ...
    print(f"spatialregularizationweight λ = {lambda_reg}, IoU threshold = {iou_thresh}")
    ...
    for epoch in range(num_epochs):
        ...
        if avg_loss < best_loss:
            ...
            print(f"savebestmodel：{ckpt_path}（loss：{best_loss:.4f}）")

        with open(train_log_path, "a") as f:
            f.write(f"{epoch + 1},{avg_loss:.6f},{best_loss:.6f},{current_lr:.6f}\n")

        print(f"Epoch {epoch + 1} | averageloss：{avg_loss:.4f} | ：{current_lr:.6f}")
...
def train_pcl_imu(config, exp_name="pcl_imu"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[PCL/OICR] trainingdevice：{device}")
    print(f"[PCL/OICR] experiment：{exp_name}")
    ...
    print(f"[PCL/OICR] training：{len(train_dataset)}")

    train_backbone = bool(config["training"].get("train_backbone", False))
    if not backbone_loaded:
        train_backbone = True
        print("[PCL/OICR-Train] pretraining ->  train_backbone=True（randomtraining）")
    ...
    for epoch in range(num_epochs):
        ...
        current_lr = scheduler.get_last_lr()[0]
        print(f"[PCL/OICR] Epoch {epoch+1} | averageloss: {epoch_loss:.4f}")
    ...
    print(f"[PCL/OICR] trainingcomplete，best_loss={best_loss:.4f}")
    return ckpt_path
