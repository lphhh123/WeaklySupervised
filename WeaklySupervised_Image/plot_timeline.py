...
def _load_gt_segments(gt_path, video_id, subset="test"):
    """
    Message ActivityNet Message GT:
    data["database"][video_id]["annotations"] Message:
      - label: str
      - segment: [t0, t1]  (Message；MessageOpportunityMessage)
    """
...
def _load_pred_segments(pred_path, video_id):
    """
    Message predictions_test_full.json:
    data["results"][video_id] Message:
      - label: str
      - score: float
      - segment: [t0, t1]
    """
...
def _build_label2color(labels):
    ...
    """
        Message tab20, tab20b, tab20c Message 60 Message、
        Message。
        """
...
def merge_same_label(segs, gap=0.0):
    """MessagelabelMessage/Message(gapMessage)Message"""
    by = defaultdict(list)
...
def suppress_diff_labels_drop(pred_segs):
    """
    MessagelabelMessage：Message，Message（Message）
    """
...
if all_labels:
    ...
    print(f"Message video_id '{video_id}' Message。")
