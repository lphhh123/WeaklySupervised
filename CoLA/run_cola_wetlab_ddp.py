...
# [适配] 引入 RWHAR 数据集
from WSDDN.WETLAB.dataset_wetlab_ws import WeaklyWetlabDataset
...
def map_config_to_cola_cfg(user_config, fold):
    c = edict()
    c.DATASET_NAME = 'WETLAB'
    c.MODAL = 'imu'
    c.FEATS_FPS = user_config['fps']  # 50
    c.FEATS_DIM = user_config['in_channels']  # 3
    c.NUM_CLASSES = user_config['num_classes']  # 12

    c.NUM_SEGMENTS = int(user_config['clip_sec'] * c.FEATS_FPS)
    c.UP_SCALE = 1

    # 预训练权重路径
    ckpt_name = f"wetlab_{user_config['pretrained_model_name']}_pretrained_loso_sbj_{fold}.pth"
...
def train_fold_ddp(config, fold, rank, local_rank, world_size):
    ...
    for epoch in range(config["training"]["num_epochs"]):
        ...
        for data, label in loader:
            ...
            # 对 CAS 进行 Sigmoid 映射到 0-1 空间
            cas_prob = torch.sigmoid(cas)

            # 计算平均激活强度：值越小，预测越稀疏（背景越多）
            # torch.mean 会计算整个 Tensor [B, T, C] 的平均值
...
def eval_fold_dual_mode_rank0(config, fold, ckpt_path, save_dir, device):
    ...
    test_ds = WeaklyWetlabDataset(
        dataset_dir=config["dataset_dir"], loso_json=f"loso_sbj_{fold}.json", mode="test_window",
        fps=config["fps"], num_sensors=config["in_channels"], clip_sec=60,  # 测试时读取全长
        normalize=True
    )
...
def main():
    ...
    base_config = {
        ...
        "result_root": "./output_wetlab_cola_ddp",
