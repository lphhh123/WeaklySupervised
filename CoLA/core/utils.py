...
def result2json(result, class_dict):
    ...
    for i in range(len(result)):
        for j in range(len(result[i])):
            label_id = result[i][j][0]
            if label_id not in class_idx2name: continue
            line = {
                'label': class_idx2name[label_id],
                'score': float(result[i][j][1]),
                'segment': [float(result[i][j][2]), float(result[i][j][3])]
            }
...
def set_path(config):
    if config.MODE == 'train':
        config.EXP_NAME = 'output_ddp'
        config.MODEL_PATH = os.path.join(config.EXP_NAME, 'checkpoints')
        config.LOG_PATH = os.path.join(config.EXP_NAME, 'logs')
        makedirs(config.MODEL_PATH)
        makedirs(config.LOG_PATH)
    elif config.MODE == 'test':
        config.EXP_NAME = 'output_test'
    config.OUTPUT_PATH = config.EXP_NAME
