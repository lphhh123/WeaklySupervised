...
from builder_models import build_wsddn_imu_model, build_wscnet_imu_model, build_pcl_oicr_imu_model, build_pclHead_model, \
    build_oicrPALoss_imu_model
...
def train_wsddn_imu(config, exp_name="wsddn"):
    ...
    # 4. 加载训练数据集（注意这里改成 use_airpods=use_airpods）
...
def train_oicrPALoss_imu(config, exp_name="oicr_pa_imu"):
    import os
    import torch
    import torch.optim as optim
    from torch.utils.data import DataLoader
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[OICR-PA] 训练设备：{device}")
    print(f"[OICR-PA] 实验名：{exp_name}")

    # ===== 统一读取 use_airpods / in_channels =====
    use_airpods = bool(config["training"].get("use_airpods", False))
    in_channels = 30 + (6 if use_airpods else 0)
    print(f"[OICR-PA] use_airpods={use_airpods}, in_channels={in_channels}")

    # ===== 预训练 ckpt 是否存在（决定 train_backbone 强制开关）=====
    spec = get_pretrained_spec(config["model"]["pretrained_name"])
    pretrain_ckpt = spec.get("ckpt")
    backbone_loaded = (pretrain_ckpt is not None and os.path.exists(pretrain_ckpt))
    print(f"[OICR-PA] pretrained_name={config['model']['pretrained_name']}")
    print(f"[OICR-PA] backbone_ckpt={pretrain_ckpt}")
    print(f"[OICR-PA] backbone_loaded={backbone_loaded}")
    ...
    _, _, new_to_action = load_label_mapping(config["path"]["mapping_path"])
    num_classes = len(new_to_action)
    print(f"[OICR-PA] num_classes={num_classes}, T_global={T_global}")

    # ===== 3) build OICR-PA head =====
    ...
    print(f"[OICR-PA] 训练集样本数：{len(train_dataset)}")

    # ===== 5) backbone 是否参与训练 =====
    train_backbone = bool(config["training"].get("train_backbone", False))
    if not backbone_loaded:
        train_backbone = True
        print("[OICR-PA] 预训练不存在 -> 强制 train_backbone=True（随机初始化并参与训练）")
    print(f"[OICR-PA] train_backbone={train_backbone}")
    ...
    backbone_lr = float(config["training"].get("backbone_lr", config["training"]["lr"] * 0.1))
    param_groups = [{"params": model.parameters(), "lr": float(config["training"]["lr"])}]
    if train_backbone:
        param_groups.append({"params": pretrained_backbone.parameters(), "lr": backbone_lr})
    ...
    for epoch in range(num_epochs):
        ...
        pbar = tqdm(train_loader, desc=f"[OICR-PA] Epoch {epoch+1}/{num_epochs}")
        for sample_30s, proposal_boxes, labels in pbar:
            sample_30s = sample_30s.to(device)          # [B,30/36,2048]
            proposal_boxes = proposal_boxes.to(device)  # [B,P,2]
            labels = labels.to(device)                  # [B,C]
            ...
            if losses is None:
                raise ValueError("[OICR-PA] outputs must contain 'losses' in training mode.")
        ...
        avg_loss = epoch_loss / len(train_dataset)
        current_lr = scheduler.get_last_lr()[0]
        print(f"[OICR-PA] Epoch {epoch+1} | avg_loss={avg_loss:.4f} | lr={current_lr:.6f}")
        ...
        if avg_loss < best_loss:
            ...
            save_dict = {
                ...
                # 记录关键超参（可选但推荐）
                "stage0_boost": float(config["model"]["pcl"].get("stage0_boost", 3.0)),
                "pa_mode": str(config["model"]["pcl"].get("pa_mode", "sigmoid")),
                "enhance_weight": bool(config["model"]["pcl"].get("enhance_weight", False)),
            }
            if train_backbone:
                save_dict["backbone_state_dict"] = pretrained_backbone.state_dict()

            torch.save(save_dict, ckpt_path)
            print(f"[OICR-PA] -> New best, save to {ckpt_path} (best_loss={best_loss:.4f})")

        scheduler.step()

    print(f"[OICR-PA] 训练完成 best_loss={best_loss:.4f}")
    return ckpt_path


def train_wscnet_imu(config, exp_name="wscnet_imu"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 1) 构造训练集（跟 WSDDN 一样，只是我们不再用 proposals）
    train_dataset = WeaklySupervisedXRFV2DatasetTrain(
        dataset_dir=config["path"]["train_dataset_path"],
        mapping_path=config["path"]["mapping_path"],
        split="train",
    ...
    # 类别数（和 old_to_new_action 长度一致，这里也可以直接用 config["training"]["num_class"]）
    num_classes = len(train_dataset.old_to_new_action)

    # 2) 构建 WSCNet-IMU 模型并加载预训练 backbone
    model = build_wscnet_imu_model(config, num_classes=num_classes)
    model = model.to(device)

    # 3) 损失函数 & 优化器
    # video-level 是 multi-label（一个 30s 里可能多类），所以用 BCEWithLogits
    ...
    for epoch in range(num_epochs):
        ...
        for sample_30s, proposal_boxes, video_label in tqdm(
            train_loader, desc=f"[WSCNet] Epoch {epoch+1}/{num_epochs}"
        ):
            # sample_30s: [B, 30, 2048]
            # video_label: [B, C] (multi-hot)
            x = sample_30s.to(device).float()
            y = video_label.to(device).float()

            optimizer.zero_grad()

            logits_det, logits_cls, class_maps, att_global = model(x)

            # 两个分支各一个 BCE loss，类似原 WSCNet:
            loss_det = criterion(logits_det, y)
            loss_cls = criterion(logits_cls, y)
            loss = 0.5 * loss_det + 0.5 * loss_cls
        ...
        current_lr = scheduler.get_last_lr()[0]
        print(f"[WSCNet] Epoch {epoch+1}/{num_epochs}  Loss: {epoch_loss:.4f}")
        # === 将训练损失记录到文件 ===
        with open(train_log_path, "a") as f:
            f.write(f"{epoch + 1},{epoch_loss:.6f},{best_loss:.6f},{current_lr:.6f}\n")
        ...
        if epoch_loss < best_loss:
            ...
            print(f"  -> New best, save to {ckpt_path}")

        scheduler.step()

    print(f"[WSCNet] Training done. Best loss={best_loss:.4f}")
    return ckpt_path

def train_pclHead(config, exp_name="pclHead"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f" 训练设备：{device}")
    print(f" 实验名：{exp_name}")
    ...
    # 1. 加载 backbone（仅提特征，不训练）
    pretrained_backbone, T_global = load_pretrained_backbone(
        pretrained_name=config["model"]["pretrained_name"],
        device=device,
        in_channels=in_channels,
    )
    print(f"特征提取器：{pretrained_backbone.__class__.__name__}")
    ...
    _, _, new_to_action = load_label_mapping(config["path"]["mapping_path"])
    num_classes = len(new_to_action)
    print(f" num_classes = {num_classes}, T_global = {T_global}")
    ...
    for epoch in range(num_epochs):
        ...
        # 方案二：直接喂 global_feat
        # for feat, proposal_boxes, labels in train_loader:
        #     feat = feat.to(device)  # [B,512,T]
        #     proposal_boxes = proposal_boxes.to(device)  # [B,P,2]
        #     labels = labels.to(device)  # [B,C]
        #
        #     outputs = model(feat, proposal_boxes, labels)
            ...
            epoch_loss += total_loss.item() * sample_30s.size(0)
            # epoch_loss += total_loss.item() * feat.size(0)
            ...
            torch.save(
                {
                    ...
                    "epoch": epoch + 1,
                    "num_classes": num_classes,
                    "use_airpods": use_airpods,   # ★ 建议存一下，避免测试时忘开关
                    "in_channels": in_channels,
                },
                ckpt_path
            )
